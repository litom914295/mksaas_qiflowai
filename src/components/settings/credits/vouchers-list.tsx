...
export function VouchersList() {
  ...
  const onGift = async (id: string) => {
    try {
      const res = await fetch('/api/vouchers/prepare-gift', {
        method: 'POST',
  ...
  if (loading)
    return <div className="text-sm text-muted-foreground">加载礼券中...</div>;
  if (!vouchers.length)
    return <div className="text-sm text-muted-foreground">暂无礼券</div>;
        ...
        <Card key={v.id} className="">
          ...
          <CardContent className="flex items-center justify-between text-sm">
            <div>
              <div>用途：{v.action}</div>
              <div>
                剩余：{v.unitsRemain} / {v.unitsTotal}
              </div>
              <div>
                有效期：
                {v.expireAt
                  ? new Date(v.expireAt).toLocaleDateString()
                  : '长期有效'}
              </div>
            </div>
