...
export function PillarsDetail({ data }: PillarsDetailProps) {
  ...
  const getKongwang = (dayStem: string, dayBranch: string) => {
    const key = Object.keys(kongwangMap).find(
      (k) =>
  ...
  const kongwang = getKongwang(
    base.pillars.day.heavenlyStem,
    base.pillars.day.earthlyBranch
  );
  ...
  const detectRelations = () => {
    ...
    for (let i = 0; i < branches.length; i++) {
      for (let j = i + 1; j < branches.length; j++) {
        if (
          branchRelations.clash[
            branches[i] as keyof typeof branchRelations.clash
          ] === branches[j]
        ) {
          relations.push({
            type: '相冲',
    ...
    for (let i = 0; i < branches.length; i++) {
      for (let j = i + 1; j < branches.length; j++) {
        if (
          branchRelations.harm[
            branches[i] as keyof typeof branchRelations.harm
          ] === branches[j]
        ) {
          relations.push({
            type: '相害',
    ...
    for (let i = 0; i < branches.length; i++) {
      for (let j = i + 1; j < branches.length; j++) {
        if (
          branchRelations.combine[
            branches[i] as keyof typeof branchRelations.combine
          ] === branches[j]
        ) {
          relations.push({
            type: '六合',
          ...
          <div className="mt-6 grid grid-cols-2 md:grid-cols-4 gap-4 p-4 rounded-lg bg-gradient-to-r from-blue-50 to-purple-50">
            ...
            <div className="text-center">
              ...
              <div className="text-lg font-bold text-green-700">
                {['甲', '丙', '戊', '庚', '壬'].includes(base.dayMaster.chinese)
                  ? '阳'
                  : '阴'}
              </div>
            </div>
            ...
            <CardContent>
              ...
              <div className="mt-6 p-4 rounded-lg bg-purple-50 border-2 border-purple-200">
                ...
                <div className="text-sm text-gray-700 space-y-1">
                  <p>
                    • <strong>本气</strong>: 地支中力量最强的天干,占主导地位
                  </p>
                  <p>
                    • <strong>中气</strong>: 地支中力量中等的天干,起辅助作用
                  </p>
                  <p>
                    • <strong>余气</strong>: 地支中力量最弱的天干,影响较小
                  </p>
            ...
            <CardContent>
              <div className="space-y-4">
                ...
                <div className="p-4 rounded-lg bg-blue-50 border-2 border-blue-200">
                  ...
                  <div className="text-sm text-gray-700 space-y-1">
                    <p>
                      • <strong>六合</strong>: 地支相合,和谐相助,有利于运势
                    </p>
                    <p>
                      • <strong>相冲</strong>: 地支对冲,气场相克,易生变动
                    </p>
                    <p>
                      • <strong>相害</strong>: 地支相害,暗中不利,需要化解
                    </p>
                    <p>
                      • <strong>相刑</strong>: 地支相刑,内在冲突,需要调和
                    </p>
                  </div>
                </div>
              </div>
