    ...
    if (flyingStars?.combined) {
      ...
      for (let row = 0; row < 3; row++) {
        for (let col = 0; col < 3; col++) {
          ...
          // 获取飞星数据
          const starData = flyingStars.combined.find(
            (s: any) => s.palace === palace
          );
          if (starData) {
            // 山星（左上）
      ...
      <CardHeader>
        <CardTitle className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            ...
            {facingDirection && (
              <Badge variant="default">向 {facingDirection}</Badge>
            )}
          </div>
        </CardTitle>
      </CardHeader>
