...
interface TimePickerProps {
  ...
  autoCloseOnSelect?: boolean; // 选择完成后自动关闭
}
...
export function TimePicker({
  ...
  const formatTime = (): string => {
    if (!value || selectedHour === null || selectedMinute === null) return '';
...
// 更新时间（closeAfter 表示本次选择完成，需自动关闭）
  const updateTime = (hour: number, minute: number, am?: boolean, closeAfter: boolean = false) => {
    ...
    if (autoCloseOnSelect && closeAfter) {
      setIsOpen(false);
    }
  };
          ...
          {use12Hour && (
            <div className="mb-4">
              ...
              <div className="flex gap-2">
                <button
onClick={() => updateTime(selectedHour ?? 12, selectedMinute ?? 0, true, true)
                  className={`
                ...
                <button
onClick={() =>
                    updateTime(selectedHour ?? 12, selectedMinute ?? 0, false, true)
                  }
                  className={`
                    flex-1 p-2 text-sm rounded border hover:bg-blue-50 transition-colors
                    ${!isAM ? 'bg-blue-500 text-white border-blue-500' : 'border-gray-300'}
          ...
          <div className="mb-4">
            ...
            <div className="grid grid-cols-2 gap-2">
              <button
onClick={() => updateTime(9, 0, use12Hour ? true : undefined, true)
