...
export function GuestAnalysisPage({}: GuestAnalysisPageProps = {}) {
  ...
  // 监听URL参数，从罗盘页面返回时接收方向值
  useEffect(() => {
    const orientation = searchParams.get('orientation');
    const compassUsed = searchParams.get('compassUsed');

    if (orientation && compassUsed === 'true') {
      const orientationValue = Number.parseFloat(orientation);
      setCompassOrientation(orientationValue);

      // 清除URL参数，保持页面干净
      const url = new URL(window.location.href);
      url.searchParams.delete('orientation');
      url.searchParams.delete('compassUsed');
