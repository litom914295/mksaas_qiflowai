...
import { Compass, Home, Navigation, Sparkles } from 'lucide-react';
...
const SimpleCompass = dynamic(
  () => import('@/components/compass/simple-compass'),
  {
    ssr: false,
    loading: () => (
      <div className="animate-pulse bg-gray-200 rounded-full w-96 h-96" />
    ),
  }
);
...
const StandardFloorPlan = dynamic(
  () =>
    import('./standard-floor-plan').then((mod) => ({
      default: mod.StandardFloorPlan,
    })),
  { ssr: false }
);
...
export const HouseDataForm = memo(function HouseDataForm({
  ...
  // 跳转到独立罗盘页面
  const handleUseCompass = () => {
    const returnUrl = window.location.pathname;
    const currentDir = compassOrientation || 0;
    router.push(
      `/compass-picker?returnUrl=${encodeURIComponent(returnUrl)}&direction=${currentDir}`
    );
  };
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <div className="flex gap-2">
                ...
                <Button
                  type="button"
                  onClick={handleUseCompass}
                  className="bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-700 hover:to-blue-700 text-white px-4 whitespace-nowrap"
                  title="使用罗盘定位"
                >
