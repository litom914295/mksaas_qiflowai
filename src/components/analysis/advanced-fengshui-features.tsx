      ...
      <div className="space-y-4">
        ...
        <div className="p-4 bg-green-50 rounded-lg">
          <h4 className="font-medium text-gray-900 mb-2">
            当前时辰：{currentHourName}
          </h4>
          <p className="text-sm text-gray-700">
            不同时辰的能量场不同，影响该时段的运势。
            {selectedHour >= 5 && selectedHour < 7
              ? ' 卯时：阳气初升，适合学习工作。'
              : ''}
            {selectedHour >= 11 && selectedHour < 13
              ? ' 午时：阳气最盛，适合重要决策。'
              : ''}
            {selectedHour >= 17 && selectedHour < 19
              ? ' 酉时：金气当令，适合理财投资。'
              : ''}
            {selectedHour >= 21 && selectedHour < 23
              ? ' 亥时：水气当令，适合休息养生。'
              : ''}
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {directions.map((dir) => {
          const cell = plate.find(
            (c) => c.palace === (dir.palace as PalaceIndex)
          );
