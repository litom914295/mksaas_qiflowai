...
// 简化的九宫格组件（保留作为备用）
function SimpleNinePalaceGrid({ plate, onCellClick }: { 
  plate: Plate; 
    ...
    <div className="grid grid-cols-3 gap-2 max-w-md mx-auto">
      {[1, 2, 3, 4, 5, 6, 7, 8, 9].map((palace) => {
        ...
        return (
          ...
          >
            <div className="text-sm font-bold text-gray-800 mb-2">{palaceName}宫</div>
            {cell && (
...
// 飞星详情卡片
function StarDetailCard({ cell, palaceName }: { 
  cell: PlateCell; 
  palaceName: string;
}) {
    ...
    <Card className="p-4">
      ...
      <div className="grid grid-cols-3 gap-3 mb-4">
        <div className="text-center">
          ...
          <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold mx-auto ${getStarColor(cell.periodStar || 1)}`}>
            {cell.periodStar}
          </div>
        </div>
        <div className="text-center">
          <div className="text-sm text-gray-600 mb-1">山盘</div>
          <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold mx-auto ${getStarColor(cell.mountainStar)}`}>
            {cell.mountainStar}
          </div>
        </div>
        <div className="text-center">
          <div className="text-sm text-gray-600 mb-1">向盘</div>
          <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold mx-auto ${getStarColor(cell.facingStar)}`}>
            {cell.facingStar}
          </div>
        </div>
      </div>
      ...
      <div className="space-y-6">
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <span className="text-lg font-semibold text-gray-900">整体格局</span>
            <Badge 
...
// 建议组件
function Recommendations({ summary }: { summary: FlyingStarExplanation['summary'] }) {
  return (
    <Card className="p-6">
...
export function FlyingStarAnalysis({ fengshuiResult, fengshuiExplanation }: FlyingStarAnalysisProps) {
  ...
  const handleViewChange = (view: 'overview' | 'palaces' | 'geju' | 'advice') => {
    setActiveTab(view);
  };
    ...
    <div className="space-y-6">
      ...
      <Card className="p-6 bg-gradient-to-r from-blue-50 to-purple-50">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h2 className="text-2xl font-bold text-gray-900">玄空飞星风水分析</h2>
