...
const ComprehensiveAnalysis: React.FC = () => {
  ...
  // 生成风水分析
  const generateFengshuiAnalysis = (house: HouseInfo): AnalysisResult['fengshui'] => {
    ...
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      ...
      {step === 'house' && (
        <Card>
          ...
          <CardContent>
            <Tabs value={houseInfo.inputMethod} onValueChange={(value) => setHouseInfo(prev => ({ ...prev, inputMethod: value as any }))}>
              ...
              <TabsContent value="compass" className="space-y-4">
                <div className="text-center">
                  <p className="text-sm text-gray-600 mb-4">点击罗盘设置房屋朝向</p>
                  <FengShuiCompass
                    width={350}
                    height={350}
                    onDirectionChange={(direction) => setHouseInfo(prev => ({ ...prev, direction }))}
                    theme="compass"
                  />
                  <p className="mt-4 text-lg font-medium">
                    当前朝向: {houseInfo.direction.toFixed(0)}° 
                    ({['正北', '东北', '正东', '东南', '正南', '西南', '正西', '西北'][Math.floor(houseInfo.direction / 45)]})
                  </p>
                </div>
              </TabsContent>
              ...
              <TabsContent value="manual" className="space-y-4">
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setHouseInfo(prev => ({ ...prev, direction: parseInt(e.target.value) || 0 }))}
                    min="0"
                    max="360"
              ...
              <TabsContent value="upload" className="space-y-4">
                <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center">
                  ...
                  <p className="text-sm text-gray-600 mb-4">上传房屋平面图，我们将自动分析朝向</p>
                  <input
                    type="file"
                    accept="image/*,.pdf"
            ...
            <div className="flex space-x-4 mt-6">
              <Button onClick={performAnalysis} className="w-full" disabled={isAnalyzing}>
      ...
      {step === 'analysis' && analysisResult && (
        <div className="space-y-6">
            ...
            <CardContent>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                <div className="text-center p-4 bg-blue-50 rounded-lg">
                  ...
                  <div className="text-xl font-bold text-blue-600">{analysisResult.bazi.year}</div>
                </div>
                <div className="text-center p-4 bg-green-50 rounded-lg">
                  <div className="text-sm text-gray-600">月柱</div>
                  <div className="text-xl font-bold text-green-600">{analysisResult.bazi.month}</div>
                </div>
                <div className="text-center p-4 bg-yellow-50 rounded-lg">
                  <div className="text-sm text-gray-600">日柱</div>
                  <div className="text-xl font-bold text-yellow-600">{analysisResult.bazi.day}</div>
                </div>
                <div className="text-center p-4 bg-purple-50 rounded-lg">
                  <div className="text-sm text-gray-600">时柱</div>
                  <div className="text-xl font-bold text-purple-600">{analysisResult.bazi.hour}</div>
                </div>
              </div>
          ...
          <Card>
            ...
            <CardContent>
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between">
                      <span>朝向:</span>
                      <span className="font-medium">{analysisResult.fengshui.direction}°</span>
                    </div>
                    <div className="flex justify-between">
                      <span>玄空排盘:</span>
                      <span className="font-medium">{analysisResult.fengshui.xuankong}</span>
                    </div>
                  </div>
                </div>
                <div>
              ...
              <div className="mt-6">
                ...
                <p className="text-gray-700 mb-4">{analysisResult.fengshui.analysis}</p>
          ...
          <div className="flex space-x-4">
            <Button variant="outline" onClick={() => setStep('house')} className="flex-1">
              重新分析
            </Button>
