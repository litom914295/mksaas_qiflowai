...
// 飞星含义解释
const STAR_MEANINGS = {
  1: { name: '一白贪狼星', element: '水', nature: '吉', meaning: '智慧、学业、官运' },
  2: { name: '二黑巨门星', element: '土', nature: '凶', meaning: '疾病、是非、破财' },
  3: { name: '三碧禄存星', element: '木', nature: '凶', meaning: '是非、口舌、争斗' },
  4: { name: '四绿文曲星', element: '木', nature: '吉', meaning: '文昌、学业、智慧' },
  5: { name: '五黄廉贞星', element: '土', nature: '凶', meaning: '灾祸、意外、破财' },
  6: { name: '六白武曲星', element: '金', nature: '吉', meaning: '权力、地位、偏财' },
  7: { name: '七赤破军星', element: '金', nature: '凶', meaning: '破财、盗贼、口舌' },
  8: { name: '八白左辅星', element: '土', nature: '吉', meaning: '财运、事业、健康' },
  9: { name: '九紫右弼星', element: '火', nature: '吉', meaning: '喜庆、桃花、名声' },
};
...
function StarExplanation({ star }: { star: number }) {
  ...
  const colorClass = isAuspicious ? 'text-green-800 bg-green-100' : 'text-red-800 bg-red-100';
    ...
    <div className="flex items-center gap-3 p-3 bg-white rounded-lg border">
      ...
      <div className="flex-1">
        <div className="flex items-center gap-2 mb-1">
          ...
          <Badge variant={isAuspicious ? "default" : "destructive"} className="text-xs">
            {starInfo.nature}
          </Badge>
        </div>
        <div className="text-sm text-gray-600">
          <span className="font-medium">五行：</span>{starInfo.element} | 
          <span className="font-medium ml-2">含义：</span>{starInfo.meaning}
        </div>
      </div>
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {geju.types.map((gejuType, index) => {
          const explanation = GEJU_EXPLANATIONS[gejuType as keyof typeof GEJU_EXPLANATIONS];
          ...
          return (
            <Card key={index} className="p-4">
              <Collapsible open={isExpanded} onOpenChange={(open) => setExpandedGeju(open ? gejuType : null)}>
                <CollapsibleTrigger asChild>
                  <Button variant="ghost" className="w-full justify-between p-0 h-auto">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm text-gray-600">{explanation.description}</span>
                    </div>
...
// 建议解释组件
function RecommendationExplanation({ summary }: { summary: FlyingStarExplanation['summary'] }) {
      ...
      {sections.map((section) => {
        ...
        return (
          <Card key={section.key} className="p-4">
            <Collapsible open={isExpanded} onOpenChange={(open) => setExpandedSection(open ? section.key : null)}>
              <CollapsibleTrigger asChild>
                <Button variant="ghost" className="w-full justify-between p-0 h-auto">
                  <div className="flex items-center gap-2">
                ...
                <div className="space-y-2">
                  {section.items.map((item, index) => (
                    <div key={index} className="flex items-start gap-3 p-3 bg-gray-50 rounded-lg">
                      <div className={`w-2 h-2 rounded-full mt-2 flex-shrink-0 bg-${section.color}-500`}></div>
...
function StarKnowledgeBase() {
  ...
  return (
    <Card className="p-6">
      ...
      {selectedStar && (
        <StarExplanation star={selectedStar} />
      )}
