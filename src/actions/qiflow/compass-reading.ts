...
import { readCompassSmart, type SensorData, type CompassConfig } from '@/lib/qiflow/compass'
...
export async function compassReadingAction(formData: FormData) {
  const parsed = InputSchema.safeParse({
    ...
    magnetometer: formData.get('magnetometer') ? JSON.parse(formData.get('magnetometer') as string) : undefined,
    gyroscope: formData.get('gyroscope') ? JSON.parse(formData.get('gyroscope') as string) : undefined,
  ...
  try {
    ...
    if (parseFloat(confidence) < 0.5) {
      console.warn(`[compass] Low confidence reading for user ${userId}: ${confidence}`)
      // 可以返回一个特定的错误码或警告
    }
