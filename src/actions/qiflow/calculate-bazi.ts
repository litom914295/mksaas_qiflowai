...
import { handleDegradation, handleManualInput } from '@/lib/qiflow/degradation-handler'
...
export async function calculateBaziAction(formData: FormData) {
  ...
  try {
    ...
    qiflowLogger.warn('Bazi calculation - sensitive content detected', { traceId, action: 'bazi-calculate', inputName: input.name })
  ...
  try {
    ...
    qiflowLogger.error('Bazi calculation - insufficient credits', { traceId, action: 'bazi-calculate', userId, creditsNeeded: creditsUsed, error: error instanceof Error ? error.message : String(error) })
  ...
  try {
    ...
    qiflowLogger.error('Bazi calculation - algorithm error', { traceId, action: 'bazi-calculate', userId, error: error instanceof Error ? error.message : String(error) })
    ...
    if (degradationResponse.shouldReject) {
      qiflowLogger.info('Bazi calculation - degraded due to low confidence', { traceId, action: 'bazi-calculate', userId, confidence: degradationResponse.confidence, degradationReason: degradationResponse.degradationResult?.reason })
      return {
        ok: false as const,
  ...
  if (degradationResponse.shouldReject) {
    ...
    qiflowLogger.error('Bazi calculation - database insert failed', { traceId, action: 'bazi-calculate', userId, error: e instanceof Error ? e.message : String(e) })
  }
