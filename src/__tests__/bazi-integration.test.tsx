...
describe('八字分析前端集成测试', () => {
  ...
  describe('API调用集成', () => {
    it('应该成功调用API并显示结果', async () => {
      ...
      const { getByLabelText, getByRole, getByText } = render(<BaziAnalysisPage />);
  ...
  describe('结果展示交互', () => {
    ...
    it('应该切换选项卡显示不同内容', async () => {
      const { getByRole, getByText } = render(<BaziAnalysisResult result={mockBaziAnalysis.data} />);
    ...
    it('应该展开和折叠详细内容', async () => {
      const { getByRole, queryByText } = render(<BaziAnalysisResult result={mockBaziAnalysis.data} />);
    ...
    it('应该复制分析结果', async () => {
      ...
      const { getByRole } = render(<BaziAnalysisResult result={mockBaziAnalysis.data} />);
    ...
    it('应该导出PDF报告', async () => {
      ...
      // 验证API调用
      expect(fetch).toHaveBeenCalledWith('/api/bazi/export', expect.any(Object));
      
      // 验证下载触发
      await waitFor(() => {
        const link = document.querySelector('a[download]');
        expect(link).toHaveAttribute('download', expect.stringContaining('.pdf'));
      });
    });
  });
  ...
  describe('历史记录功能', () => {
    ...
    it('应该加载历史记录', async () => {
      localStorage.setItem('bazi_history', JSON.stringify([
        {
          id: '1',
          name: '王五',
          date: '1985-05-05',
  ...
  describe('无障碍测试', () => {
    ...
    it('应该有正确的ARIA标签', () => {
      const { container } = render(<BaziAnalysisResult result={mockBaziAnalysis.data} />);
