 ...
 * 户型图状态默认值
 */
export const DEFAULT_FLOORPLAN_STATE: Omit<FloorplanState, 'id' | 'imageData' | 'imageType' | 'createdAt' | 'updatedAt'> = {
  rotation: 0,
  scale: 1,
  position: { x: 0, y: 0 },
  showOverlay: true,
...
export const FloorplanStorageKeys = {
  /** 生成用户户型图缓存键 */
  user: (userId: string, analysisId: string) => `floorplan_${userId}_${analysisId}`,
