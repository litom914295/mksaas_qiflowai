import { FlyingStar, Mountain, PalaceIndex, Plate } from './types';

// 洛书九宫顺序（中心→西北→西→东北→南→北→西南→东→东南）
export const LUOSHU_ORDER: PalaceIndex[] = [5, 6, 7, 8, 9, 1, 2, 3, 4] as PalaceIndex[];
...
export function generateXiangpan(
  ...
  const baguaYinYang = getBaguaYinYang(bagua);
  const isShun = (baguaYinYang === '阳' && xiangYuanLong === '天') || 
