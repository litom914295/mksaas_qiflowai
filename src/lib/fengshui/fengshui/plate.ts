import { FlyingStar, PalaceIndex, Plate } from './types';

// Placeholder mapping refined later by true flying rules; keeps determinism.
export function generatePlate(seed: number, applyTiGua = false, applyFanGua = false): Plate {
  const cells: Plate = [];
...
const LUOSHU_ORDER: PalaceIndex[] = [5, 6, 7, 8, 9, 1, 2, 3, 4] as PalaceIndex[];

export function generatePeriodPlateByLuoShu(period: FlyingStar, forward = true): Plate {
  const plate: Plate = [];
  let current = period;
