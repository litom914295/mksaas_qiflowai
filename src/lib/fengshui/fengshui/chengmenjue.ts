...
export function identifyChengmenPositions(
  ...
  // 查找适用的城门诀规则
  const applicableRules = CHENGMEN_RULES.filter(
  ...
  for (const rule of applicableRules) {
    ...
    if (rule.triggerCondition.starCombination && isMatched) {
      const targetCell = plate.find(
        (cell) => cell.palace === rule.chengmenPosition
      );
      if (targetCell) {
        const {
          mountain,
          facing,
          period: periodStar,
        } = rule.triggerCondition.starCombination;
    ...
    if (isMatched) {
      ...
      if (targetCell) {
        ...
        if (
          targetCell.mountainStar === nextStar ||
          targetCell.facingStar === nextStar
        ) {
          strength += 1;
        }
      }
...
export function analyzeChengmenjue(
  ...
  for (const position of chengmenPositions) {
    ...
    const taboos = generateChengmenTaboos(
      position.palace,
      position.rule.chengmenType
    );
