...
// 二十四山向度数映射
export const MOUNTAIN_DEGREES: Record<
  Mountain,
...
export class TwentyFourMountainsAnalyzer {
   ...
   * 分析坐向组合
   */
  analyzeSittingFacing(
    sitting: Mountain,
    facing: Mountain
  ): SittingFacingAnalysis {
    ...
    > = {
      子午: {
        compatibility: 'excellent',
        description: '子山午向，正南正北，君子正位',
      },
      卯酉: {
        compatibility: 'excellent',
        description: '卯山酉向，正东正西，日月对照',
      },
    ...
    // 计算各方面影响
    const effects = {
      wealth: this.calculateEffectScore(
        'wealth',
        sittingElement,
        facingElement,
        baseScore
      ),
      health: this.calculateEffectScore(
        'health',
        sittingElement,
        facingElement,
        baseScore
      ),
      relationships: this.calculateEffectScore(
        'relationships',
        sittingElement,
        facingElement,
        baseScore
      ),
      career: this.calculateEffectScore(
        'career',
        sittingElement,
        facingElement,
        baseScore
  ...
  analyzeDragonCave(params: {
    ...
    // 水势分析
    const waterPattern = this.analyzeWaterPattern(
      waterIncoming,
      waterOutgoing,
      cavePosition
    );
    ...
    // 综合评分
    const scores = [
      dragonQuality.quality === 'excellent'
        ? 90
        : dragonQuality.quality === 'good'
          ? 60
          : 30,
      caveEnergy.energy === 'strong'
        ? 90
        : caveEnergy.energy === 'moderate'
          ? 60
          : 30,
      sandBalance.balance === 'balanced' ? 90 : 50,
      waterPattern.pattern === 'auspicious'
        ? 90
        : waterPattern.pattern === 'neutral'
          ? 50
          : 20,
    ];
    ...
    if (sandBalance.balance !== 'balanced') {
      recommendations.push(
        `调整左右护砂，${sandBalance.balance === 'left-heavy' ? '加强右侧' : '加强左侧'}防护`
      );
    }
  ...
  private analyzeCaveEnergy(
    ...
    const relationship = this.analyzeMountainRelationship(
      dragonDirection,
      position
    );
    ...
    if (
      ...
      relationship.relationship === 'generating'
    ) {
      energy = 'strong';
    } else if (
      relationship.relationship === 'conflicting' ||
      relationship.relationship === 'controlling'
    ) {
      energy = 'weak';
    }
  ...
  private analyzeWaterPattern(
    ...
    if (incoming && outgoing && cavePosition) {
      ...
      const incomingRelation = this.analyzeMountainRelationship(
        incoming,
        cavePosition
      );
      const outgoingRelation = this.analyzeMountainRelationship(
        cavePosition,
        outgoing
      );

      if (
        incomingRelation.relationship === 'generating' &&
        (outgoingRelation.relationship === 'generating' ||
          outgoingRelation.relationship === 'compatible')
      ) {
        pattern = 'auspicious';
      } else if (
        incomingRelation.relationship === 'conflicting' ||
  ...
  recommendOptimalFacing(params: {
    ...
    const analysis =
      `根据命理分析，年命属${yearElement}，宜坐${MOUNTAIN_ELEMENTS[bestChoice]}向，` +
  ...
  private isGenerating(element1: Element, element2: Element): boolean {
    ...
    return (
      generating[element1] === element2 || generating[element2] === element1
    );
  }
 ...
 * 快速分析坐向吉凶
 */
export function quickAnalyzeSittingFacing(
  sitting: Mountain,
  facing: Mountain
): SittingFacingAnalysis {
