import { Plate, PalaceIndex, FlyingStar, Yun, GejuType, GejuAnalysis, Mountain } from './types';
...
export function checkShuangxingHuixiang(
  ...
  const isShuangxing = xiangCell.mountainStar === period && xiangCell.facingStar === period;
...
export function checkShuangxingHuizuo(
  ...
  const isShuangxing = zuoCell.mountainStar === period && zuoCell.facingStar === period;
...
// 检查全局合十格局
export function checkQuanjuHeshi(plate: Plate): { isMatch: boolean; description: string } {
  let heshiCount = 0;
  
  for (const cell of plate) {
    const sum = (cell.mountainStar || 0) + (cell.facingStar || 0) + (cell.periodStar || 0);
    if (sum === 10) {
      heshiCount++;
    }
  }
...
// 检查伏吟格局
export function checkFuyin(plate: Plate): { isMatch: boolean; description: string; locations: string[] } {
  ...
  if (fuyinLocations.length > 0) {
    return { isMatch: true, description: '伏吟格局', locations: fuyinLocations };
  }
...
// 检查反吟格局
export function checkFanyin(plate: Plate): { isMatch: boolean; description: string; locations: string[] } {
  ...
  if (fanyinLocations.length > 0) {
    return { isMatch: true, description: '反吟格局', locations: fanyinLocations };
  }
...
export function analyzeGeju(
  ...
  if (zuoCell && xiangCell) {
    const lianzhuSanban = checkLianzhuSanban(period, zuoCell.mountainStar, xiangCell.facingStar);
    if (lianzhuSanban.isMatch) {
      gejuTypes.push('连珠三般');
      descriptions.push(lianzhuSanban.description);
    }
    
    const fumusanban = checkFumuSanban(period, zuoCell.mountainStar, xiangCell.facingStar);
  ...
  // 判断整体吉凶
  const favorableGeju = ['旺山旺水', '双星会向', '双星会坐', '全局合十', '对宫合十', '连珠三般', '父母三般'];
  const unfavorableGeju = ['上山下水', '山星伏吟', '向星伏吟', '单宫伏吟', '单宫反吟', '替卦反伏吟'];
  
  const hasFavorable = gejuTypes.some(type => favorableGeju.includes(type));
  const hasUnfavorable = gejuTypes.some(type => unfavorableGeju.includes(type));
