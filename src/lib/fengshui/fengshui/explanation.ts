...
import type { FlyingStar, GejuAnalysis, PalaceIndex, Plate } from './types';
...
export function generatePalaceExplanation(
  ...
  const tianpan = {
    ...
    meaning: getStarMeaning(cell.periodStar!, palaceStatus.tianpan.status)
      .meaning,
  };
  ...
  const shanpan = cell.mountainStar
    ? {
        ...
        name: getStarMeaning(cell.mountainStar, palaceStatus.shanpan!.status)
          .name,
        meaning: getStarMeaning(cell.mountainStar, palaceStatus.shanpan!.status)
          .meaning,
      }
    : null;
  ...
  const xiangpan = cell.facingStar
    ? {
        ...
        name: getStarMeaning(cell.facingStar, palaceStatus.xiangpan!.status)
          .name,
        meaning: getStarMeaning(cell.facingStar, palaceStatus.xiangpan!.status)
          .meaning,
      }
    : null;
...
export function generateFlyingStarExplanation(
  ...
  if (favorablePalaces.length > 0) {
    recommendations.push(
      `建议在${favorablePalaces.join('、')}方位布置重要功能区`
    );
  }
  ...
  // 检查是否有五黄星相关的凶险格局
  if (
    geju.types.some((type) =>
      ['上山下水', '山星伏吟', '向星伏吟'].includes(type)
    )
  ) {
