...
import type { FlyingStar, PalaceIndex, Plate, StarStatus } from './types';
  ...
  for (const cell of plate) {
    ...
    if (xiangpan && shengwangStars.includes(xiangpan)) {
      ...
      if (shanpan && xiangpan) {
        ...
        if (
          CAIWEI_COMBINATIONS.includes(combination) ||
          CAIWEI_COMBINATIONS.includes(reverseCombination)
        ) {
...
// 获取各宫位的生旺死煞退状态
export function getPalaceStatus(
  plate: Plate,
  period: FlyingStar
): Record<
  PalaceIndex,
...
// 获取九星寓意
export function getStarMeaning(
  star: FlyingStar,
  status: StarStatus
