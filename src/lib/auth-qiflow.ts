...
export async function onQiflowUserCreated(user: User) {
  ...
  try {
    ...
    console.error(
      `Failed to initialize QiFlow profiles for user ${userId}:`,
      error
    );
    // 不抛出错误，避免影响用户注册流程
  }
...
export async function grantQiflowSignupBonus(userId: string) {
  if (
    ...
    try {
      ...
      console.error(
        `Failed to grant QiFlow signup bonus to user ${userId}:`,
        error
      );
    }
  }
 ...
 */
export async function onUserCreated(params: {
  userId: string;
