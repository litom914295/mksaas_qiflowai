...
interface RouterHealthStatus {
  ...
  providers: Record<
    string,
    { healthy: boolean; lastCheck: number; errorCount: number }
  >;
...
const calculateBackoffDelay = (attempt: number): number => {
  const delay = ROUTER_CONFIG.RETRY_DELAY_BASE * 2 ** attempt;
...
export const createRouter = (policy?: RoutingPolicy, budget?: CostBudget) => {
    ...
    const selected = order
      ...
      .filter((client): client is ProviderClient => {
        ...
        if (status.circuitBreakerOpen) {
          const now = Date.now();
          if (
            now - status.circuitBreakerOpenTime >
            ROUTER_CONFIG.CIRCUIT_BREAKER_TIMEOUT
          ) {
            // 尝试恢复
        ...
        try {
          ...
          const timeoutPromise = new Promise<boolean>((_, reject) =>
            setTimeout(
              ...
              ROUTER_CONFIG.HEALTH_CHECK_TIMEOUT
            )
          );
          ...
          console.error(
            `[AI路由器] ${client.name} 请求失败 (重试 ${retryCount}/${ROUTER_CONFIG.MAX_RETRIES}):`,
            error
          );
  ...
  const getHealthStatus = async (): Promise<RouterHealthStatus> => {
    ...
    const healthyProviders = Object.values(providerHealths).filter(
      (p) => p.healthy
    ).length;
    const errorRate =
      totalRequests > 0
