...
export class ContextManager {
  ...
  updateHistory(
    ...
    context.relatedTopics = this.mergeTopics(
  ...
  selectTemplate(
    ...
    if (
      intent.includes('room') ||
      intent.includes('bedroom') ||
      intent.includes('kitchen')
    ) {
      return 'fengshui.room_analysis';
    }

    if (
      intent.includes('bazi') ||
      intent.includes('八字') ||
      intent.includes('命理')
    ) {
      return 'bazi.explain';
  ...
  private analyzeIntent(
    ...
    // 教育意图
    if (
      messageLower.includes('什么是') ||
      messageLower.includes('解释') ||
      messageLower.includes('原理') ||
      messageLower.includes('如何理解')
    ) {
    ...
    // 咨询意图
    if (
      messageLower.includes('建议') ||
      messageLower.includes('怎么办') ||
      messageLower.includes('如何改善') ||
      messageLower.includes('推荐')
    ) {
    ...
    // 分析意图
    if (
      messageLower.includes('分析') ||
      messageLower.includes('看看') ||
      messageLower.includes('评估') ||
      messageLower.includes('判断')
    ) {
    ...
    // 房间相关意图
    if (
      messageLower.includes('房间') ||
      messageLower.includes('卧室') ||
      messageLower.includes('客厅') ||
      messageLower.includes('厨房')
    ) {
      intents.push('room');
    }
  ...
  private getRelevantTerms(topic?: string): TermExplanation[] {
    ...
    const relevantTerms = Object.values(CULTURAL_TERMS_DB).filter(
      (term) =>
        term.category === topic ||
  ...
  updateUserProfile(
    sessionId: string,
    updates: Partial<ConversationContext['userProfile']>
  ): void {
    const context = this.contexts.get(sessionId);
    if (!context) return;
...
export class TermExplainer {
  ...
  static searchTerms(query: string): TermExplanation[] {
    ...
    for (const explanation of Object.values(CULTURAL_TERMS_DB)) {
      if (
        explanation.term.includes(query) ||
