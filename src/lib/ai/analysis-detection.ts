...
export function extractAnalysisParams(
  ...
  if (yearMatch || monthMatch || dayMatch) {
    params.birthDate = {
      ...
      month: monthMatch ? Number.parseInt(monthMatch[1]) : undefined,
      day: dayMatch ? Number.parseInt(dayMatch[1]) : undefined,
    };

    if (hourMatch) {
      if (hourMatch[1]) {
        params.birthDate.hour = Number.parseInt(hourMatch[1]);
      } else if (hourMatch[2]) {
        // 转换时辰为小时
