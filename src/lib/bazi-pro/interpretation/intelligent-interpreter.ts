...
import { BaziChart, InterpretationResult, UserLevel } from '../types/index';
...
import { TenGodsRelations } from '../core/analyzer/ten-gods-relations';
...
export class IntelligentInterpreter {
  ...
  private tenGodsAnalyzer: TenGodsRelations;
  ...
  constructor() {
    ...
    this.tenGodsAnalyzer = new TenGodsRelations();
  }
  ...
  async interpret(
    ...
    // 收集所有分析数据
    const analysisData = await this.collectAnalysisData(chart);
    
    // 根据用户水平生成解读
    const interpretation = await this.generateInterpretation(analysisData, options);
    
    // 个性化调整
    const personalized = this.personalizeContent(interpretation, chart, options);
  ...
  private async collectAnalysisData(chart: BaziChart) {
    return {
      strength: this.strengthAnalyzer.calculateDayMasterStrength(chart),
      elements: this.strengthAnalyzer.analyzeElements(chart),
      yongshen: this.yongshenAnalyzer.analyze(chart),
      pattern: this.patternDetector.analyzePatterns(chart),
      shensha: this.shenshaCalculator.analyzeShenSha(chart),
      tenGods: this.tenGodsAnalyzer.analyzeTenGods(chart)
    };
  }
  ...
  private async generateSummary(data: any, options: InterpretationOptions) {
    ...
    const overview = this.createOverview(pattern, yongshen, strength, options.userLevel);
   ...
   * 解读性格特征
   */
  private async interpretPersonality(data: any, options: InterpretationOptions) {
    const { tenGods, elements, pattern } = data;
    
    const traits = this.analyzePersonalityTraits(tenGods, elements, options.userLevel);
  ...
  private async interpretCareer(data: any, options: InterpretationOptions) {
    ...
    const suitable = this.identifySuitableCareers(pattern, tenGods, options.userLevel);
  ...
  private async interpretWealth(data: any, options: InterpretationOptions) {
    ...
    const advice = this.generateWealthAdvice(yongshen, pattern, options.userLevel);
   ...
   * 解读感情关系
   */
  private async interpretRelationships(data: any, options: InterpretationOptions) {
    const { tenGods, shensha, pattern } = data;
    
    const love = this.analyzeLoveRelationships(tenGods, shensha, options.userLevel);
  ...
  private async interpretHealth(data: any, options: InterpretationOptions) {
    ...
    const vulnerabilities = this.identifyHealthVulnerabilities(elements, shensha);
    const wellness = this.generateWellnessAdvice(elements, pattern, options.userLevel);
  ...
  private async interpretFortune(data: any, options: InterpretationOptions) {
    ...
    const guidance = this.generateFortuneGuidance(yongshen, pattern, options.userLevel);
   ...
   * 解读精神层面
   */
  private async interpretSpirituality(data: any, options: InterpretationOptions) {
    const { pattern, elements, yongshen } = data;
    
    const insights = this.generateLifeInsights(pattern, elements, options.userLevel);
  ...
  private personalizeContent(
    ...
    } else if (options.userLevel === 'expert') {
  ...
  private createOverview(pattern: any, yongshen: any, strength: number, level: UserLevel): string {
    if (level === 'beginner') {
  ...
  private analyzePersonalityTraits(tenGods: any, elements: any, level: UserLevel): string[] {
    const traits: string[] = [];
  ...
  private identifySuitableCareers(pattern: any, tenGods: any, level: UserLevel): string[] {
    const careers: string[] = [];
    
    // 根据格局推荐职业
    switch (pattern.mainPattern.type) {
      case 'zhengguan':
  ...
  private analyzeWorkStyle(pattern: any, tenGods: any): string[] {
    ...
    if (pattern.mainPattern.type.includes('jian') || pattern.mainPattern.type.includes('yue')) {
      styles.push('独立自主', '追求卓越', '勇于担当');
    } else if (pattern.mainPattern.type.includes('cong')) {
  ...
  private generateCareerAdvice(yongshen: any, pattern: any): string[] {
    return [
      ...
      pattern.recommendations.find((r: string) => r.includes('事业')) || '稳步发展，把握机遇'
    ];
  }
  ...
  private generateWealthAdvice(yongshen: any, pattern: any, level: UserLevel): string[] {
    if (level === 'beginner') {
      return [
        '理性消费，量入为出',
        '建立应急储备金',
      ...
      return [
        ...
        pattern.recommendations.find((r: string) => r.includes('财')) || '稳健理财'
      ];
    }
  }

  private analyzeLoveRelationships(tenGods: any, shensha: any, level: UserLevel): string[] {
    const love: string[] = [];
    
    // 检查桃花
    const peachBlossom = shensha.jiShen.find((s: any) => s.name.includes('桃花'));
    if (peachBlossom) {
      love.push(level === 'beginner' ? '异性缘佳' : '命带桃花，异性缘旺');
    }
    
    // 检查孤辰寡宿
    const lonely = shensha.xiongShen.find((s: any) => s.name.includes('孤') || s.name.includes('寡'));
    if (lonely) {
  ...
  private generateWellnessAdvice(elements: any, pattern: any, level: UserLevel): string[] {
    if (level === 'beginner') {
      return [
        '保持规律作息',
        '均衡饮食营养',
      ...
      // 根据五行给出具体建议
      const weak = Object.entries(elements)
  ...
  private analyzeCurrentFortune(yongshen: any, shensha: any): string[] {
    ...
    fortune.push(`当前运势${shensha.summary.totalJiShen > shensha.summary.totalXiongShen ? '吉利' : '需谨慎'}`);
  ...
  private generateFortuneGuidance(yongshen: any, pattern: any, level: UserLevel): string[] {
    return yongshen.recommendations;
  }

  private generateLifeInsights(pattern: any, elements: any, level: UserLevel): string[] {
    if (level === 'beginner') {
      return [
        '人生是一场修行',
        '顺应自然规律',
  ...
  private simplifyTerminology(interpretation: ComprehensiveInterpretation): ComprehensiveInterpretation {
    // 简化专业术语为通俗易懂的语言
    return interpretation;
  }

  private addTechnicalDetails(interpretation: ComprehensiveInterpretation): ComprehensiveInterpretation {
    // 添加更多专业技术细节
    return interpretation;
  }
  ...
  private getSimpleCareer(data: any): string {
    return data.pattern.recommendations.find((r: string) => r.includes('事业') || r.includes('适合')) || '多元发展';
  }
