...
export class DayunLiuNianCalculator {
  private readonly GAN_LIST = ['甲', '乙', '丙', '丁', '戊', '己', '庚', '辛', '壬', '癸'];
  private readonly ZHI_LIST = ['子', '丑', '寅', '卯', '辰', '巳', '午', '未', '申', '酉', '戌', '亥'];
  ...
  private calculateQiYunAge(
    ...
    // 计算相差天数
    const diffDays = Math.abs(jieQi.getTime() - birthDate.getTime()) / (1000 * 60 * 60 * 24);
  ...
  public calculateLiuNian(
    ...
    for (let i = -2; i <= 5; i++) {
      if (i !== 0 && i !== 1) { // 排除当年和明年（已单独计算）
        recentYears.push(
          this.calculateSingleLiuNian(
            fourPillars,
            currentYear + i,
            age + i
          )
  ...
  private calculateSingleLiuNian(
    ...
    const { highlights, warnings } = this.generateLiuNianTips(fourPillars, gan, zhi);
