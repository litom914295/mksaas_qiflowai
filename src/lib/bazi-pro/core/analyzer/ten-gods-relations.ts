...
import { FourPillars } from '../calculator/four-pillars';
import { TenGods, tenGodsCalculator } from '../calculator/ten-gods';
...
export interface TenGodBalance {
  ...
  // 平衡分析
  balance: {
    type: 'balanced' | 'official_heavy' | 'seal_heavy' | 'wealth_heavy' | 'food_heavy' | 'rob_heavy';
    description: string;
    suggestions: string[];
  };
}
...
export class TenGodRelationAnalyzer {
   ...
   * 收集所有十神
   */
  private collectAllTenGods(tenGods: TenGods): Map<string, number> {
    ...
    for (const god of stems) {
      if (god) {
        godsCount.set(god, (godsCount.get(god) || 0) + 1);
      }
    }
    ...
    for (const branchGods of branches) {
      for (const hiddenGod of branchGods.hiddenGods) {
        godsCount.set(
          hiddenGod.god,
          (godsCount.get(hiddenGod.god) || 0) + hiddenGod.strength
        );
      }
    }
   ...
   * 识别十神组合
   */
  private identifyCombinations(allGods: Map<string, number>): TenGodCombination[] {
  ...
  private analyzeTenGodBalance(
    ...
    const wealthFood = 
      (allGods.get('正财') || 0) + (allGods.get('偏财') || 0);
    
    const sealLibrary = 
      (allGods.get('正印') || 0) + (allGods.get('偏印') || 0);
    
    const robWealth = 
      (allGods.get('比肩') || 0) + (allGods.get('劫财') || 0);
    
    const injuryFood = 
      (allGods.get('食神') || 0) + (allGods.get('伤官') || 0);
