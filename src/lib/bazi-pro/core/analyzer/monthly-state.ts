...
export class MonthlyStateAnalyzer {
  ...
  private calculateStates(monthElement: Element): MonthlyState['states'] {
    ...
    for (const element of elements) {
      states[element] = this.getElementState(element, monthElement);
    }
   ...
   * 获取单个五行在月令中的状态
   */
  private getElementState(element: Element, monthElement: Element): WuxingState {
    // 当令者旺
  ...
  private getGeneratingElement(element: Element): Element {
    for (const [key, value] of Object.entries(MonthlyStateAnalyzer.GENERATING_CYCLE)) {
      if (value === element) {
  ...
  private getControllingElement(element: Element): Element {
    for (const [key, value] of Object.entries(MonthlyStateAnalyzer.CONTROLLING_CYCLE)) {
      if (value === element) {
        return key as Element;
      }
    }
  ...
  private calculateCoefficients(
    ...
    for (const element of elements) {
      coefficients[element] = MonthlyStateAnalyzer.STATE_COEFFICIENTS[states[element]];
    }
  ...
  public isTransitionMonth(monthBranch: string): boolean {
    return MonthlyStateAnalyzer.SEASONAL_MONTHS.transition.includes(monthBranch);
  }
  ...
  public getSeason(monthBranch: string): string {
    for (const [season, months] of Object.entries(MonthlyStateAnalyzer.SEASONAL_MONTHS)) {
  ...
  public calculateAdjustedStrength(
    ...
    const coefficient = monthlyState.coefficients[element];
  ...
  public getDayMasterTendency(
    ...
    const monthlyState = this.analyzeMonthlyState(monthBranch);
    const state = monthlyState.states[dayMasterElement];
    const coefficient = monthlyState.coefficients[dayMasterElement];
