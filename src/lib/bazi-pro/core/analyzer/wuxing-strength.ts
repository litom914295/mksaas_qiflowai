...
import { FourPillars, Pillar } from '../calculator/four-pillars';
...
export class WuxingStrengthAnalyzer {
  ...
  private applyMonthlyCoefficients(
    ...
    for (const [element, coefficient] of Object.entries(coefficients)) {
      const elementKey = this.getElementKey(element);
      const adjustment = (strength[elementKey] * coefficient) - strength[elementKey];
  ...
  private calculateInteractions(
    ...
    for (const element of elements) {
      ...
      if (strength[generatorKey] > 0) {
        const bonus = strength[generatorKey] * 0.2; // 被生加20%
        strength[elementKey] += bonus;
    ...
    for (const element of elements) {
      ...
      if (strength[controllerKey] > 0) {
        const penalty = strength[controllerKey] * 0.15; // 被克减15%
  ...
  private normalizeStrength(strength: any): WuxingStrength {
    const total = strength.wood + strength.fire + strength.earth + 
                  strength.metal + strength.water;
  ...
  public calculateDayMasterStrength(
    ...
    let supportingStrength = wuxingStrength[elementKey];
    
    for (const element of supportingElements) {
      const key = this.getElementKey(element);
      supportingStrength += wuxingStrength[key];
    }
    ...
    // 分析因素
    if (hiddenStemsAnalyzer.hasRoot(dayMaster, [
      fourPillars.year.zhi,
      fourPillars.month.zhi,
      fourPillars.day.zhi,
      fourPillars.hour.zhi
    ...
    if (wuxingStrength[elementKey] > 20) {
      factors.push('比劫帮身');
    }
    
    const generator = this.getGeneratingElementFor(dayMasterElement);
    const generatorKey = this.getElementKey(generator);
    if (wuxingStrength[generatorKey] > 15) {
      factors.push('印星生助');
    }
