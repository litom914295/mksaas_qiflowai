...
import { FourPillars, fourPillarsCalculator } from '../calculator/four-pillars';
import { TenGods, tenGodsCalculator } from '../calculator/ten-gods';
import { hiddenStemsAnalyzer, HiddenStem } from './hidden-stems';
import { 
  WuxingStrength, 
  DayMasterStrength,
...
export class BaziAnalyzer {
  ...
  public async analyze(params: {
    ...
    // Step 2: 计算十神
    const tenGods = tenGodsCalculator.calculate(
      fourPillars,
      params.gender
    );
    
    // Step 3: 五行力量分析
    const wuxingStrength = wuxingStrengthAnalyzer.calculateWuxingStrength(fourPillars);
    ...
    // Step 8: 事业财运分析
    const career = this.analyzeCareer(
      tenGodAnalysis,
      pattern,
      godPreferences
    );
    ...
    return {
      basicInfo: {
        ...
        solarTime: fourPillars.solarTime || '',
        lunarDate: fourPillars.lunarDate || ''
      },
  ...
  private analyzePattern(
    ...
    return {
      ...
      subPatterns,
      special,
      description: description || `${mainPattern}，${dayMasterStrength.factors.join('，')}`,
      score
    };
  }
  ...
  private calculateScores(
    ...
    if (tenGodAnalysis.combinations.length > 0) {
      const avgScore = tenGodAnalysis.combinations.reduce(
        (sum, c) => sum + c.score, 0
