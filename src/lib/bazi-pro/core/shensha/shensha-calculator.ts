...
export class ShenShaCalculator {
  ...
  private calculateTianDe(chart: BaziChart): ShenShaResult | null {
    ...
    if (stems.includes(tianDeStem as any)) {
      return {
        type: ShenShaType.JI_SHEN,
        name: '天德贵人',
  ...
  private calculateYueDe(chart: BaziChart): ShenShaResult | null {
    ...
    if (stems.includes(yueDeStem as any)) {
      return {
        type: ShenShaType.JI_SHEN,
        name: '月德贵人',
  ...
  private calculateFuXing(chart: BaziChart): ShenShaResult | null {
    ...
    if (branches.includes(fuXingBranch as any)) {
      return {
        type: ShenShaType.JI_SHEN,
        name: '福星贵人',
        description: '主福气深厚',
  ...
  private calculateTianChu(chart: BaziChart): ShenShaResult | null {
    ...
    if (branches.includes(tianChuBranch as any)) {
      return {
        type: ShenShaType.JI_SHEN,
        name: '天厨贵人',
        description: '主衣食丰足',
  ...
  private calculateWangShen(chart: BaziChart): ShenShaResult | null {
    ...
    if (branches.includes(wangShenBranch as any)) {
      return {
        type: ShenShaType.XIONG_SHEN,
        name: '亡神',
  ...
  private calculateJieSha(chart: BaziChart): ShenShaResult | null {
    ...
    if (branches.includes(jieShaBranch as any)) {
      return {
        type: ShenShaType.XIONG_SHEN,
        name: '劫煞',
        description: '主破财劫难',
  ...
  private calculateZaiSha(chart: BaziChart): ShenShaResult | null {
    ...
    if (branches.includes(zaiShaBranch as any)) {
      return {
        type: ShenShaType.XIONG_SHEN,
        name: '灾煞',
        description: '主有灾祸意外',
  ...
  private calculateLiuE(chart: BaziChart): ShenShaResult | null {
    ...
    if (branches.includes(liuEBranch as any)) {
      return {
        type: ShenShaType.XIONG_SHEN,
        name: '六厄',
        description: '主困厄阻碍',
  ...
  private calculateGouJiao(chart: BaziChart): ShenShaResult | null {
    ...
    const hasGou = branches.includes(gou as any);
    const hasJiao = branches.includes(jiao as any);
  ...
  private calculateYuanChen(chart: BaziChart): ShenShaResult | null {
    ...
    if (yuanChenBranch && branches.includes(yuanChenBranch as any)) {
      return {
        type: ShenShaType.XIONG_SHEN,
        name: '元辰',
  ...
  private calculateGuChenGuaSu(chart: BaziChart): ShenShaResult | null {
    ...
    const hasGuChen = branches.includes(guChen as any);
    const hasGuaSu = branches.includes(guaSu as any);
    ...
    if (hasGuChen || hasGuaSu) {
      ...
      return {
        ...
        effects: [...effects, '感情不顺', '晚婚迹象', '六亲缘薄'],
