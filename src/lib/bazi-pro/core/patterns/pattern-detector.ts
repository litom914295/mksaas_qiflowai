...
import { BaziChart, Element, PatternResult, PatternType, PatternDetail } from '../../types/index';
import { WuxingStrengthCalculator } from '../analyzer/wuxing-strength';
import { TenGodsRelations } from '../analyzer/ten-gods-relations';

export interface PatternAnalysisResult {
  mainPattern: PatternType;        // 主格局类型
  subPatterns: PatternType[];      // 辅助格局
...
export class PatternDetector {
  private strengthCalculator: WuxingStrengthCalculator;
  private tenGods: TenGodsRelations;

  constructor() {
    this.strengthCalculator = new WuxingStrengthCalculator();
    this.tenGods = new TenGodsRelations();
  }
  ...
  analyzePatterns(chart: BaziChart): PatternAnalysisResult {
    ...
    // 分析十神分布
    const tenGodsDistribution = this.tenGods.analyzeTenGods(chart);
    
    // 检测各种格局
    const patterns: PatternDetail[] = [];
    
    // 1. 正格检测
    const regularPattern = this.detectRegularPattern(chart, dayMasterStrength, tenGodsDistribution);
    ...
    // 4. 专旺格检测
    const dominantPattern = this.detectDominantPattern(chart, dayMasterStrength);
    ...
    // 计算格局纯粹度
    const strength = this.calculatePatternStrength(chart, mainPattern, patterns);
    
    // 生成建议和分析
    const recommendations = this.generateRecommendations(mainPattern, patterns, chart);
   ...
   * 检测从格（从财、从官、从儿等）
   */
  private detectFollowPattern(chart: BaziChart, strength: number): PatternDetail | null {
    // 日主极弱，小于20%
    if (strength > 20) return null;
    
    const dominantElement = this.findDominantElement(chart);
    const relationship = this.getElementRelationship(chart.pillars.day.heavenlyStem, dominantElement);
    
    switch (relationship) {
      case 'wealth':
        return {
          type: PatternType.CONG_CAI,
          name: '从财格',
   ...
   */
  private detectDominantPattern(chart: BaziChart, strength: number): PatternDetail | null {
  ...
  private isKuiGangPattern(chart: BaziChart): boolean {
    ...
    const dayPillar = chart.pillars.day.heavenlyStem + chart.pillars.day.earthlyBranch;
  ...
  private hasTransform(stems: string[], stem1: string, stem2: string, element: string): boolean {
  ...
  private getElementRelationship(dayMaster: string, element: string): string {
    ...
    return relationships[dayElement]?.[element] || 'none';
  }
  ...
  private determineMainPattern(patterns: PatternDetail[]): PatternType {
    if (patterns.length === 0) return PatternType.NORMAL;
  ...
  private generateRecommendations(
    ...
    const mainPatternDetail = patterns.find(p => p.type === mainPattern);
    if (mainPatternDetail) {
      recommendations.push(`您的命格为${mainPatternDetail.name}，${mainPatternDetail.description}`);
      recommendations.push(...mainPatternDetail.characteristics.map(c => `• ${c}`));
    }
    
    // 根据格局类型给出具体建议
    switch (mainPattern) {
      case PatternType.JIAN_LU:
      case PatternType.YUE_REN:
        recommendations.push('建议：身强宜泄，可取食伤生财为用');
        recommendations.push('事业：适合自主创业或担任管理职位');
        break;
      
      case PatternType.CONG_CAI:
        recommendations.push('建议：顺从财势，以财为用神');
        recommendations.push('事业：适合经商、投资理财等与财富相关的领域');
        break;
      
      case PatternType.CONG_GUAN:
  ...
  private identifyConflicts(patterns: PatternDetail[], chart: BaziChart): string[] {
    const conflicts: string[] = [];
    
    // 检查格局之间的冲突
    if (patterns.some(p => p.type === PatternType.JIAN_LU) &&
        patterns.some(p => p.type === PatternType.CONG_CAI)) {
  ...
  private analyzePotentials(mainPattern: PatternType, chart: BaziChart): string[] {
