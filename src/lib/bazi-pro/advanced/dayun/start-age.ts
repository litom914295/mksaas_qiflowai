...
import type { BaziChart, Gender, HeavenlyStem } from '../../types/index';
...
export class StartAgeCalculator {
  ...
  calculateQiYun(chart: BaziChart, gender: Gender): QiYunInfo {
    ...
    const daysToJieQi = Math.abs(
      Math.floor(
        (nearestJieQi.getTime() - birthTime.getTime()) / (1000 * 60 * 60 * 24)
      )
    );
  ...
  private getYearJieQi(year: number): Array<{ name: string; date: Date }> {
    ...
    for (const jie of jieList) {
      ...
      // 使用农历适配器获取精确的节气时间
      const jieQiDate = lunarAdapter.getSolarTermDate(actualYear, jie.name);
  ...
  calculatePreciseQiYunTime(chart: BaziChart, gender: Gender): Date {
    ...
    // 加上起运年数
    preciseStartTime.setFullYear(
      preciseStartTime.getFullYear() + qiYunInfo.startAge
    );
