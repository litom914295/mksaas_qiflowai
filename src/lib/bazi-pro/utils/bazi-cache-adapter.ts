...
export class BaziCacheAdapter {
  ...
  async warmupCache(dates: BaziDate[]): Promise<void> {
    const keys = dates.map((date) => this.generateDateKey('fourPillars', date));
  ...
  private generateDateKey(prefix: string, date: BaziDate): string {
    return MemoryCache.generateKey({
      ...
      day: date.day,
      hour: date.hour,
      minute: (date as any).minute || 0,
      isLunar: (date as any).isLunar,
    });
  }
  ...
  return (
    ...
    descriptor.value = async function (...args: any[]) {
      ...
      try {
        ...
        if (duration > 100) {
          console.warn(
            `[Performance Warning] ${methodName} took ${duration.toFixed(2)}ms`
          );
        }
        ...
        const duration = performance.now() - start;
        console.error(
          `[Performance Error] ${methodName}: ${duration.toFixed(2)}ms`,
          error
        );
        throw error;
      }
