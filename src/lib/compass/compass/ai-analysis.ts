...
import { CompassUtil } from './feng-shui-engine';
import { AIAnalysisResult } from './feng-shui-types';
...
export class FengShuiAIAnalysis {
  ...
  async analyzeDirection(direction: number): Promise<AIAnalysisResult> {
    ...
    // 分析吉凶
    const auspiciousness = this.analyzeAuspiciousness(direction, mountain.element);
    
    // 生成建议
    const suggestions = this.generateSuggestions(mountain, bagua, auspiciousness);
  ...
  // 分析方位吉凶
  private analyzeAuspiciousness(direction: number, element: string): 'auspicious' | 'neutral' | 'inauspicious' {
    // 基于传统风水理论的简化判断
    ...
    // 检查是否为凶方位
    const inauspiciousAngles = [22.5, 67.5, 112.5, 157.5, 202.5, 247.5, 292.5, 337.5];
  ...
  // 生成分析文本
  private generateAnalysis(mountain: any, bagua: any, auspiciousness: string): string {
    const elementDesc = this.getElementDescription(mountain.element);
    const baguaDesc = this.getBaguaDescription(bagua.name);
    const auspiciousnessDesc = this.getAuspiciousnessDescription(auspiciousness);
    
    return `当前方向为${mountain.name}山，属${bagua.name}卦，五行属${mountain.element}。${elementDesc} ${baguaDesc} ${auspiciousnessDesc}`;
  }

  // 生成建议
  private generateSuggestions(mountain: any, bagua: any, auspiciousness: string): string[] {
    const suggestions: string[] = [];
    
    // 基于五行的建议
    suggestions.push(`${mountain.name}山方位适宜进行${mountain.element}属性的活动`);
    
    // 基于八卦的建议
    suggestions.push(`${bagua.name}卦象征${bagua.meaning}，建议注意相关风水布局`);
