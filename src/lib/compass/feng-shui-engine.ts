...
export class FengShuiCompassEngine {
  ...
  getDataByAngle(layerIndex: number, angle: number): string {
    ...
    const dataIndex =
      Math.floor((layerData.startAngle + normalizedAngle) / singleAngle) %
      dataLength;
  ...
  private initLayerHeights(): void {
    ...
    for (let i = 0; i < this.getLayersLength(); i++) {
      ...
      const height = this.calculateLayerHeight(
        i,
        layerData.fontSize || this.config.defaultFontSize,
  ...
  private calculateLayerHeight(
    ...
    const availableHeight =
      (this.config.radius -
        this.config.tianChiRadius -
        this.config.scaleHeight) /
  ...
  private validateCompassData(data: LayerData[]): LayerData[] {
    return data.map((layer, index) => {
      ...
      // 验证起始角度
      if (
        validatedLayer.startAngle == null ||
        validatedLayer.startAngle > 360 ||
        validatedLayer.startAngle < 0
      ) {
        validatedLayer.startAngle = 0;
        console.warn(
          `Layer ${index}: startAngle should be between 0 and 360, set to 0`
        );
      }
      ...
      // 验证组合样式
      if (
        !['empty', 'equally', 'son'].includes(
          validatedLayer.togetherStyle || ''
        )
      ) {
...
export class CompassUtil {
  ...
  // 获取二十四山信息
  getTwentyFourMountain(angle: number): {
    name: string;
    bagua: string;
    element: string;
  } {
