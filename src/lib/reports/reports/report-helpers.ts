...
import type { LuckPillarAnalysis, ChartData } from './types';
...
export class BaziReportHelpers {
  ...
  static renderElementsChart(chartData: ChartData): string {
    const total = Object.values(chartData.data).reduce((sum, val) => sum + val, 0);
    if (total === 0) return '<p>五行数据暂不可用</p>';

    return `
      <div class="elements-chart-container">
        <div class="chart-legend">
          ${Object.entries(chartData.data).map(([element, count], index) => {
  ...
  static generateElementsAnalysis(baziAnalysis: any): string {
    ...
    const unfavorableElements = baziAnalysis.favorableElements?.unfavorable || [];

    return `
      <div class="elements-analysis">
        <div class="favorable-elements">
          <h4>✅ 有利元素</h4>
          <p>${favorableElements.map((el: string) => this.getElementName(el)).join('、')} - 建议多接触相关的颜色、方向和行业</p>
        </div>
        <div class="unfavorable-elements">
          <h4>⚠️ 需要平衡的元素</h4>
          <p>${unfavorableElements.map((el: string) => this.getElementName(el)).join('、')} - 建议适度减少相关元素的影响</p>
        </div>
      </div>
    `;
  }
  ...
  // 提取性格特质分析
  static extractPersonalityTraits(baziAnalysis: any, luckPillarsAnalysis: LuckPillarAnalysis[], birthDate: string): {
    type: string;
    strengths: Array<{name: string; description: string; context: string; example: string}>;
    challenges: Array<{name: string; solution: string}>;
    recommendations: string[];
  } {
    const currentLuck = this.getCurrentLuckPillar(luckPillarsAnalysis, birthDate);
    const personalityImpacts = currentLuck?.tenGodRelation?.personalityImpact || [];
  ...
  // 获取当前大运
  static getCurrentLuckPillar(luckPillarsAnalysis: LuckPillarAnalysis[], birthDate: string): LuckPillarAnalysis | undefined {
    const currentYear = new Date().getFullYear();
    const currentAge = currentYear - parseInt(birthDate.split('-')[0]);
    
    return luckPillarsAnalysis.find(lp => 
      currentAge >= lp.pillar.startAge && currentAge <= lp.pillar.endAge
    );
  }

  // 提取事业分析
  static extractCareerAnalysis(luckPillarsAnalysis: LuckPillarAnalysis[], birthDate: string): {
    preferredIndustries: Array<{name: string; reason: string; rating: number; timing: string}>;
  ...
  // 提取财运分析
  static extractWealthAnalysis(luckPillarsAnalysis: LuckPillarAnalysis[], birthDate: string): {
    pattern: {type: string; characteristics: string};
    opportunities: Array<{period: string; description: string; probability: string}>;
    advice: string[];
    recommendations: string[];
  } {
    const currentLuck = this.getCurrentLuckPillar(luckPillarsAnalysis, birthDate);
