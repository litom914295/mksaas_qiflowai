...
import { Point, Room } from '../image-processing/types';
import { AlignmentResult, PalaceMapping, RoomMappingResult, SpaceMappingConfig } from './types';
...
export class RoomMapper {
  ...
  async mapRoomsToPalaces(
    ...
    try {
      ...
      const roomMappings = this.mapRoomsToPalaceLayout(transformedRooms, palaceLayout);
      
      // 4. Optimize mapping results
      const optimizedMappings = this.optimizeMappings(roomMappings, palaceLayout);
      
      return optimizedMappings;

    } catch (error) {
      console.error('Room mapping failed:', error);
      throw new Error(`Room mapping failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
   ...
   */
  private createPalaceLayout(imageSize: { width: number; height: number }): Map<number, PalaceMapping> {
   ...
   * 找到最佳宫位
   */
  private findBestPalace(room: Room, palaceLayout: Map<number, PalaceMapping>): PalaceMapping | null {
  ...
  private calculatePalaceScore(room: Room, palace: PalaceMapping): number {
    ...
    const centerDistance = Math.sqrt(
      Math.pow(room.center.x - palace.center.x, 2) + 
      Math.pow(room.center.y - palace.center.y, 2)
    );
    const maxDistance = Math.sqrt(
      Math.pow(palace.bounds.maxX - palace.bounds.minX, 2) + 
      Math.pow(palace.bounds.maxY - palace.bounds.minY, 2)
    );
   ...
   * 计算房间边界
   */
  private calculateRoomBounds(room: Room): { minX: number; maxX: number; minY: number; maxY: number } {
   ...
   * 计算边界对齐得分
   */
  private calculateBoundaryAlignment(room: Room, palace: PalaceMapping): number {
    const roomBounds = this.calculateRoomBounds(room);
    
    // 检查房间边界是否与宫位边界对齐
    const xAlignment = this.calculateAxisAlignment(
      roomBounds.minX, roomBounds.maxX,
      palace.bounds.minX, palace.bounds.maxX
    );
    
    const yAlignment = this.calculateAxisAlignment(
      roomBounds.minY, roomBounds.maxY,
      palace.bounds.minY, palace.bounds.maxY
    );
  ...
  private calculateAxisAlignment(
    roomMin: number, roomMax: number,
    palaceMin: number, palaceMax: number
  ): number {
    ...
    const centerAlignment = Math.abs(roomCenter - palaceCenter) <= tolerance ? 1 : 0;
   ...
   * 计算映射置信度
   */
  private calculateMappingConfidence(room: Room, palace: PalaceMapping): number {
  ...
  private resolveConflicts(
    ...
    for (const mapping of mappings) {
      ...
      if (palaceCount === 1) {
        ...
        // 有冲突，选择最佳房间
        const palaceMappings = mappings.filter(m => m.palaceIndex === mapping.palaceIndex);
  ...
  private optimizeEmptyPalaces(
    ...
    const emptyPalaces = Array.from(palaceLayout.keys()).filter(p => !usedPalaces.has(p));
