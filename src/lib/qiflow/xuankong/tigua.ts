...
export function checkTiguaRequired(
  ...
  // 查找匹配的替卦规则
  const matchingRule = TIGUA_RULES.find(
    (rule) =>
      rule.zuo === zuo && rule.xiang === xiang && rule.period.includes(period)
  );
...
export function analyzeTiguaImpact(
  ...
  const tiguaRule = checkTiguaRequired(zuo, xiang, period, isJian);
  const fanfuyinAnalysis = checkFanfuyinTigua(
    zuo,
    xiang,
    period,
    mountainStar,
  ...
  const adjustedMountainStar = applyTiguaAdjustment(
    mountainStar,
    zuo,
    xiang,
    period,
  ...
  const adjustedFacingStar = applyTiguaAdjustment(
    facingStar,
    zuo,
    xiang,
    period,
  ...
  // 判断替卦是否改善了格局
  const originalScore = calculatePatternScore(
    originalMountainStar,
    originalFacingStar,
    period
  );
...
export function evaluateTiguaPattern(
  ...
  const tiguaAnalysis = analyzeTiguaImpact(
    zuo,
    xiang,
    period,
    mountainStar,
  ...
  if (tiguaAnalysis.hasTigua) {
    ...
    baseScore = calculatePatternScore(
      adjustedMountainStar,
      adjustedFacingStar,
      period
    );
