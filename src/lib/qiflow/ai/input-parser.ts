...
export class InputParser {
  ...
  static parseHouseInfo(text: string): HouseInfo | null {
    ...
    if (facingMatch || mountainMatch) {
      return {
        facing: facingMatch ? Number.parseInt(facingMatch[1]) : 0,
        mountain: mountainMatch ? Number.parseInt(mountainMatch[1]) : 0,
        buildYear: yearMatch ? Number.parseInt(yearMatch[1]) : undefined,
  ...
  static parseInput(text: string): ParsedInput {
    // 尝试解析生辰八字
    const birthInfo = InputParser.parseBirthInfo(text);
    if (birthInfo) {
    ...
    // 尝试解析房屋信息
    const houseInfo = InputParser.parseHouseInfo(text);
    if (houseInfo) {
      const missingFields: string[] = [];
      let confidence = 0.7;
