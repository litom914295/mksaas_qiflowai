...
import { FengShuiResult } from '../xuankong/types'
...
export function canPerformFengShuiAnalysis(baziData?: any): {
  ...
  if (!baziData) {
    return {
      canAnalyze: false,
      reason: '风水分析必须基于您的八字命理。请先提供出生信息（年月日时、性别）。'
    }
  }
...
export function personalizeFeugShuiWithBazi(
  ...
  // 基于日主确定财位
  const wealthPosition = calculatePersonalWealthPosition(dayMaster, fengShuiData)
  
  // 基于天干确定文昌位
  const studyPosition = calculatePersonalStudyPosition(dayMaster, fengShuiData)
  
  // 基于命局确定桃花位
  const romancePosition = calculatePersonalRomancePosition(baziData, fengShuiData)
  
  // 确定最佳方位（基于用神）
  const bestDirections = determineBestDirections(yongShen, favorable, fengShuiData)
 ...
 * 根据日主计算个人财位
 */
function calculatePersonalWealthPosition(dayMaster: string, fengShuiData: any): string {
  // 基于日主天干确定财位
  ...
  if (fengShuiData?.palaceEnergies) {
    // 找到八白财星所在宫位进行综合判断
    const wealthStar = fengShuiData.palaceEnergies.find((p: any) => 
      p.stars?.includes('八白') || p.description?.includes('财')
    )
 ...
 * 根据天干计算文昌位
 */
function calculatePersonalStudyPosition(dayMaster: string, fengShuiData: any): string {
 ...
 * 计算个人桃花位
 */
function calculatePersonalRomancePosition(baziData: any, fengShuiData: any): string {
  // 基于年支或日支确定桃花位
 ...
 * 确定需要化解的方位
 */
function determineAvoidPositions(unfavorable: string[], fengShuiData: any): string[] {
  ...
  if (fengShuiData?.palaceEnergies) {
    fengShuiData.palaceEnergies.forEach((palace: any) => {
      if (palace.isSha || palace.stars?.includes('五黄') || palace.stars?.includes('二黑')) {
...
function generateLayoutSuggestions(
  ...
  if (bestDirections.length > 0) {
    suggestions.push(`将办公桌或床头朝向${bestDirections[0]}方，有利于提升运势`)
    suggestions.push(`在${bestDirections.join('、')}方位摆放重要物品或进行重要活动`)
  }
  
  // 基于化解方位的建议
  if (avoidPositions.length > 0) {
    suggestions.push(`避免在${avoidPositions[0]}方位长时间停留或放置重要物品`)
    suggestions.push(`如果${avoidPositions.join('、')}方位有煞气，可用五行化解法处理`)
  }
...
function calculateOverallScore(baziData: any, fengShuiData: any): number {
  ...
  if (baziData.yongShen && fengShuiData.palaceEnergies) {
    const favorable = baziData.yongShen.favorable || []
    const goodPalaces = fengShuiData.palaceEnergies.filter((p: any) => 
      p.isAuspicious || p.stars?.some((s: string) => ['一白', '四绿', '六白', '八白', '九紫'].includes(s))
    )
