...
import { calculateLiunianStar, calculateLiuyueStar, generateLiunianPlate } from '../xuankong/liunian-analysis';
...
export async function generateMonthlyFortune(
  ...
  // 2. 八字流年流月分析
  const baziAnalysis = analyzeBaziTimeliness(input.baziChart, input.year, input.month);
  ...
  // 5. 生成各方面运势
  const forecasts = generateDetailedForecasts(flyingStarData, baziAnalysis, input.baziChart);
...
function calculateMonthlyFlyingStar(year: number, month: number) {
  ...
  const monthlyGrid = monthPlate.map(palace => {
    ...
    const auspiciousness = evaluateAuspiciousness(palace.mountainStar, palace.facingStar);
...
function evaluateAuspiciousness(
  ...
  const isAuspicious = auspiciousStars.includes(mountainStar) && auspiciousStars.includes(facingStar);
  const isInauspicious = inauspiciousStars.includes(mountainStar) || inauspiciousStars.includes(facingStar);
  const isDangerous = (mountainStar === 5 && facingStar === 2) || (mountainStar === 2 && facingStar === 5);
...
function identifyCriticalDirections(plate: Plate) {
  ...
  for (const palace of plate) {
    ...
    // 五黄二黑组合
    if ((palace.mountainStar === 5 && palace.facingStar === 2) || 
...
function analyzeBaziTimeliness(baziChart: BaziChart, year: number, month: number) {
  // 计算流年干支
  ...
  // 五行强弱变化
  const strengthAnalysis = analyzeElementStrength(baziChart, yearGanzhi, monthGanzhi);
...
function analyzeInteractions(baziChart: BaziChart, yearGanzhi: string, monthGanzhi: string) {
...
function analyzeElementStrength(baziChart: BaziChart, yearGanzhi: string, monthGanzhi: string) {
  // 返回五行力量变化
  return {
    金: 0,
    木: 0,
...
function generateDetailedForecasts(flyingStarData: any, baziAnalysis: any, baziChart: BaziChart) {
  return {
