...
import { TenGod, getTenGod } from './enhanced-ten-gods';
...
// 地支藏干详细信息（含节气精确度）
const BRANCH_HIDDEN_STEMS_DETAILED: Record<Branch, Array<{
  stem: Stem;
  element: FiveElement; 
  strength: number;
  ...
  '子': [
    { stem: '癸', element: '水', strength: 1.0, seasonalBonus: { '仲冬': 0.2, '季冬': 0.1 } }
  ],
  '丑': [
...
function getProducer(element: FiveElement): FiveElement {
  return Object.entries(PRODUCTION_CHAIN).find(([k, v]) => v === element)?.[0] as FiveElement;
}
...
// 《穷通宝鉴》调候用神表（24节气精细版）
const QIONGTONG_BAOJIAN: Partial<Record<Stem, Record<Branch, {
  primary: FiveElement[];
  ...
  '甲': {
    '寅': {
      primary: ['火'], secondary: ['水'], avoid: ['金'],
      principle: '甲木向阳，寅月虽得时令，寒气未除，最喜丙火暖局，癸水润木',
      variations: {}
    },
    '子': { primary: ['火'], secondary: ['水'], avoid: ['金'], principle: '甲木向阳，子月寒气重，最喜丙火暖局', variations: {} },
    '丑': { primary: ['火'], secondary: ['水'], avoid: ['金'], principle: '甲木向阳，丑月寒气重，最喜丙火暖局', variations: {} },
    '卯': { primary: ['火'], secondary: ['水'], avoid: ['金'], principle: '甲木向阳，卯月虽得时令，寒气未除，最喜丙火暖局', variations: {} },
    '辰': { primary: ['火'], secondary: ['水'], avoid: ['金'], principle: '甲木向阳，辰月寒气重，最喜丙火暖局', variations: {} },
    '巳': { primary: ['火'], secondary: ['水'], avoid: ['金'], principle: '甲木向阳，巳月寒气重，最喜丙火暖局', variations: {} },
    '午': { primary: ['火'], secondary: ['水'], avoid: ['金'], principle: '甲木向阳，午月寒气重，最喜丙火暖局', variations: {} },
    '未': { primary: ['火'], secondary: ['水'], avoid: ['金'], principle: '甲木向阳，未月寒气重，最喜丙火暖局', variations: {} },
    '申': { primary: ['火'], secondary: ['水'], avoid: ['金'], principle: '甲木向阳，申月寒气重，最喜丙火暖局', variations: {} },
    '酉': { primary: ['火'], secondary: ['水'], avoid: ['金'], principle: '甲木向阳，酉月寒气重，最喜丙火暖局', variations: {} },
    '戌': { primary: ['火'], secondary: ['水'], avoid: ['金'], principle: '甲木向阳，戌月寒气重，最喜丙火暖局', variations: {} },
    '亥': { primary: ['火'], secondary: ['水'], avoid: ['金'], principle: '甲木向阳，亥月寒气重，最喜丙火暖局', variations: {} }
  }
};
...
function analyzeDayMasterStrength(pillars: Pillars): {
  ...
  // 4. 地支通根强度（权重：15%）
  const earthyBranches = [pillars.year.branch, pillars.day.branch, pillars.hour.branch];
  let rootScore = 0;
  let rootExplanation = '';
  ...
  tianganCombos.forEach(([stem1, stem2]) => {
    const hasCombo = fiveHe.some(([a, b]) => 
  ...
  // 6. 季节时令影响（权重：7%）
  let seasonScore = 0;
  const seasonalBonus = monthHidden.find(h => h.element === dayElement)?.seasonalBonus || {};
  ...
  // 如果某个克制日主的五行过多，扣分
  const hostileElement = CONTROL_CHAIN[dayElement] || Object.entries(CONTROL_CHAIN).find(([k,v]) => v === dayElement)?.[0] as FiveElement;
...
function analyzeSeasonalYongshen(
  ...
  if ((monthElement === '火' && dayElement === '木') || 
...
function selectOptimalYongshen(
  ...
  if (dayStatus === 'very_weak' || dayStatus === 'weak') {
    // 扶持用神
    primaryType = 'balance';
    primaryElements = [dayElement, getProducer(dayElement)].filter(Boolean) as FiveElement[];
  ...
  const primaryTenGods = primaryElements.map(elem => {
    // 找到该五行对应的天干，然后获取十神
    const correspondingStem = Object.entries(STEM_ELEMENTS).find(([stem, element]) => element === elem)?.[0] as Stem;
    return correspondingStem ? getTenGod(pillars.day.stem, correspondingStem) : '比肩' as TenGod;
  });
  ...
  if (primaryType === 'balance' && seasonalAnalysis.needed) {
    secondaryYongshen = {
      ...
      tenGods: [getTenGod(pillars.day.stem, Object.entries(STEM_ELEMENTS).find(([s, e]) => e === seasonalAnalysis.adjustElement)?.[0] as Stem)],
      confidence: 60,
      reasoning: `辅以调候用神：${seasonalAnalysis.adjustReason}`
    };
  } else if (primaryType === 'seasonal' && (dayStatus === 'weak' || dayStatus === 'strong')) {
    const balanceElements = dayStatus === 'weak' ? [dayElement] : [getControlled(dayElement)];
    secondaryYongshen = {
      type: 'balance',
...
function generatePreferences(
  ...
  if (dayMasterAnalysis.status === 'very_weak' || dayMasterAnalysis.status === 'weak') {
    strongly_unfavorable = [CONTROL_CHAIN[dayElement], getControlled(dayElement)].filter(Boolean) as FiveElement[];
    unfavorable = [PRODUCTION_CHAIN[dayElement]].filter(Boolean) as FiveElement[];
  } else if (dayMasterAnalysis.status === 'very_strong' || dayMasterAnalysis.status === 'strong') {
    strongly_unfavorable = [dayElement, getProducer(dayElement)].filter(Boolean) as FiveElement[];
    unfavorable = [];
  } else {
    // 中性状态，忌神相对温和
  ...
  const neutral = allElements.filter(elem => 
...
function generateGuidance(
  ...
  const favorable = [...preferences.strongly_favorable, ...preferences.favorable];
  ...
  return {
    ...
    directions: favorable.flatMap(elem => elementMappings.directions[elem] || []),
    numbers: [...new Set(favorable.flatMap(elem => elementMappings.numbers[elem] || []))],
    industries: favorable.flatMap(elem => elementMappings.industries[elem] || []),
    timing: {
      favorable_months: favorable.includes('木') ? ['2月', '3月'] : 
                       favorable.includes('火') ? ['4月', '5月', '6月'] :
                       favorable.includes('土') ? ['3月', '6月', '9月', '12月'] :
                       favorable.includes('金') ? ['7月', '8月', '9月'] : 
                       favorable.includes('水') ? ['10月', '11月', '12月', '1月'] : [],
      unfavorable_months: preferences.strongly_unfavorable.includes('木') ? ['2月', '3月'] :
                         preferences.strongly_unfavorable.includes('火') ? ['4月', '5月', '6月'] :
                         preferences.strongly_unfavorable.includes('金') ? ['7月', '8月', '9月'] :
                         preferences.strongly_unfavorable.includes('水') ? ['10月', '11月', '12月', '1月'] : [],
      favorable_hours: favorable.includes('木') ? ['3-7时(卯辰)', '1-3时(寅)'] : 
                      favorable.includes('火') ? ['11-15时(午未)', '9-11时(巳)'] :
                      favorable.includes('金') ? ['15-19时(申酉)', '19-21时(戌)'] :
                      favorable.includes('水') ? ['21-1时(亥子)', '1-3时(丑)'] : [],
      unfavorable_hours: preferences.strongly_unfavorable.includes('木') ? ['3-7时', '1-3时'] :
                        preferences.strongly_unfavorable.includes('火') ? ['11-15时', '9-11时'] :
                        preferences.strongly_unfavorable.includes('金') ? ['15-19时', '19-21时'] : 
                        preferences.strongly_unfavorable.includes('水') ? ['21-1时', '1-3时'] : []
    },
    lifestyle: [
      '居住和工作环境选择有利方位',
      '服装配饰多选用有利颜色',
 ...
 * 主函数：增强型用神分析
 */
export function enhancedYongshenAnalysis(pillars: Pillars): EnhancedYongshenAnalysis {
  // 1. 分析日主强弱
  ...
  // 3. 智能选择用神
  const yongshenAnalysis = selectOptimalYongshen(pillars, dayMasterAnalysis, seasonalAnalysis);
  ...
  const pattern = {
    ...
    patternDefects: yongshenAnalysis.primary.confidence < 70 ? ['格局不够纯粹'] : [],
    patternEnhancements: yongshenAnalysis.primary.confidence >= 80 ? ['格局配合良好'] : []
  };
  ...
  const theoretical = {
    ...
    confidence: Math.round((yongshenAnalysis.primary.confidence + (yongshenAnalysis.secondary?.confidence || 0)) / (yongshenAnalysis.secondary ? 2 : 1)),
