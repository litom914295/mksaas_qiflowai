...
export function calculateMonthPillar(
  ...
  let adjustedMonth = month - 1; // 农历月份
...
export function calculateYongShen(
  ...
  const totalElements = Object.values(elementStrength).reduce((sum, val) => sum + val, 0);
  ...
  if (isStrong) {
    ...
    // 日主弱，取生扶为用
    const generatingElement = Object.entries(generating).find(([_, v]) => v === dayElement)?.[0];
  ...
  const primaryAttrs = elementAttributes[primary as keyof typeof elementAttributes] || 
                      elementAttributes['土'];
 ...
 * 计算流年
 */
export function calculateLiuNian(year: number): { stem: string; branch: string } {
 ...
 * 计算十神
 */
export function calculateShiShen(
  dayStem: string,
  targetStem: string
): string {
