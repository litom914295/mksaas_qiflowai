...
describe('日支计算准确性验证 - 已验证数据', () => {
  ...
  verifiedSamples.forEach((sample, index) => {
    it(`${sample.name}`, async () => {
      ...
      console.log(
        `期望: ${sample.expected.day} (日支: ${sample.expected.dayBranch})`
      );
  ...
  describe('连续日期一致性验证', () => {
    ...
    it('应该正确处理子时跨日边界', async () => {
      ...
      const testTimes = [
        {
          time: '2000-01-01T22:30:00',
          desc: '22:30 (当日戌时)',
          shouldBe: '戊午',
        },
        {
          time: '2000-01-01T23:00:00',
          desc: '23:00 (次日子时)',
          shouldBe: '己未',
        },
        {
          time: '2000-01-01T23:30:00',
          desc: '23:30 (次日子时)',
          shouldBe: '己未',
        },
        {
        ...
        {
          time: '2000-01-02T00:30:00',
          desc: '00:30 (当日子时)',
          shouldBe: '己未',
        },
        {
          time: '2000-01-02T01:00:00',
          desc: '01:00 (当日丑时)',
          shouldBe: '己未',
        },
      ];
      ...
      for (const test of testTimes) {
        ...
        const result = await calculator.getCompleteAnalysis();
        const actualDayPillar = result?.pillars?.day
          ? `${result.pillars.day.stem}${result.pillars.day.branch}`
          : undefined;

        console.log(
          `${test.time} (${test.desc}) => ${actualDayPillar}, 期望: ${test.shouldBe}`
        );
  ...
  describe('甲子周期验证', () => {
    it('应该在60天后回到相同的干支', async () => {
      ...
      const baseDayPillar = result1?.pillars?.day
        ? `${result1.pillars.day.stem}${result1.pillars.day.branch}`
        : undefined;
      ...
      const cycleDayPillar = result2?.pillars?.day
        ? `${result2.pillars.day.stem}${result2.pillars.day.branch}`
        : undefined;

      console.log('\n甲子周期验证:');
      console.log(
        `基准: ${baseDate.toISOString().slice(0, 10)} => ${baseDayPillar}`
      );
      console.log(
        `60天后: ${cycleDate.toISOString().slice(0, 10)} => ${cycleDayPillar}`
      );
    ...
    it('应该在一个甲子周期内产生60个不同的干支', async () => {
      ...
      const day61Pillar = result61?.pillars?.day
        ? `${result61.pillars.day.stem}${result61.pillars.day.branch}`
        : undefined;
