...
import { FengshuiInput, FengshuiOutput } from '@/app/api/fengshui/schema';
...
export function calculateFlyingStars(
  ...
  mountain: string,
  facing: string,
  isYangDirection: boolean = true
): {
  periodStars: number[][];
  mountainStars: number[][];
  facingStars: number[][];
  combinedStars: Array<{ palace: number; period: number; mountain: number; facing: number; combination: string }>;
} {
  // 运盘飞星（以当运星入中）
 ...
 * 飞星排布核心算法
 */
function flyStars(centerNumber: number, isClockwise: boolean = true): number[][] {
 ...
 * 分析飞星组合吉凶
 */
export function analyzeFlyingStarCombination(mountain: number, facing: number, period: number): {
  nature: '大吉' | '吉' | '平' | '凶' | '大凶';
  description: string;
  advice: string;
} {
  ...
  // 九运特殊组合
  const auspiciousCombos: Record<string, { nature: '大吉' | '吉'; description: string; advice: string }> = {
    '68': { nature: '大吉', description: '富贵双全', advice: '财官两旺，事业财运俱佳' },
    '86': { nature: '大吉', description: '富贵双全', advice: '财官两旺，事业财运俱佳' },
    '14': { nature: '吉', description: '文昌聚会', advice: '利于学业、考试、创作' },
    '41': { nature: '吉', description: '文昌聚会', advice: '利于学业、考试、创作' },
    '16': { nature: '吉', description: '金水相生', advice: '利于智慧、人缘、财运' },
    '61': { nature: '吉', description: '金水相生', advice: '利于智慧、人缘、财运' },
    '49': { nature: '吉', description: '风火家声', advice: '利于名声、创意、艺术' },
    '94': { nature: '吉', description: '风火家声', advice: '利于名声、创意、艺术' }
  };
  
  const inauspiciousCombos: Record<string, { nature: '凶' | '大凶'; description: string; advice: string }> = {
    '25': { nature: '大凶', description: '二五交加', advice: '主病灾，需化解土煞' },
    '52': { nature: '大凶', description: '二五交加', advice: '主病灾，需化解土煞' },
    '23': { nature: '凶', description: '斗牛煞', advice: '是非官非，需化解' },
    '32': { nature: '凶', description: '斗牛煞', advice: '是非官非，需化解' },
    '67': { nature: '凶', description: '交剑煞', advice: '口舌是非，需化解金煞' },
    '76': { nature: '凶', description: '交剑煞', advice: '口舌是非，需化解金煞' },
    '95': { nature: '凶', description: '火炼毒土', advice: '火灾、疾病，需化解' },
    '59': { nature: '凶', description: '火炼毒土', advice: '火灾、疾病，需化解' }
  };
  ...
  return { nature: '平', description: '吉凶参半', advice: '趋吉避凶，合理布局' };
}
...
export function generateFengshuiRecommendations(
  ...
  if (analysis.nature === '大吉' || analysis.nature === '吉') {
    ...
    recommendations.enhancement.push(`在${specialPositions.wealthPosition}方位摆放水晶或金属摆件增强财运`);
...
export function calculateFengshui(input: FengshuiInput): FengshuiOutput {
  // 获取山向信息
  const { mountain, facing, mountainPalace, facingPalace } = getMountainDirection(input.facing);
  ...
  // 生成建议
  const recommendations = generateFengshuiRecommendations(analysis, specialPositions);
  ...
  const output: FengshuiOutput = {
    ...
    palaceAnalysis: flyingStars.combinedStars.map(star => ({
      ...
      nature: analyzeFlyingStarCombination(star.mountain, star.facing, period).nature,
      description: analyzeFlyingStarCombination(star.mountain, star.facing, period).description
    })),
