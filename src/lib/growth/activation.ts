...
import { achievements, baziCalculations, fengshuiAnalysis, pdfAudit, referralRelationships } from '@/db/schema';
...
export async function tryMarkActivation(userId: string) {
  ...
  const tp = await db
    .select({ progress: (await import('@/db/schema')).taskProgress.progress, id: (await import('@/db/schema')).taskProgress.id })
    .from((await import('@/db/schema')).taskProgress)
    .where(and(eq((await import('@/db/schema')).taskProgress.userId, userId), eq((await import('@/db/schema')).taskProgress.taskId, 'ai_chat_rounds')))
    .limit(1);
  ...
  const dailyCountRows = await db
    .select({ c: (await import('drizzle-orm')).sql`count(*)` as unknown as number })
    .from(referralRelationships)
    .where(and(
  ...
  await addCredits({ userId: relation.referrerId!, amount: refCfg.inviterCredits, type: CREDIT_TRANSACTION_TYPE.REFERRAL_REWARD, description: '推荐奖励（推荐人）' });
  await addCredits({ userId, amount: refCfg.inviteeCredits, type: CREDIT_TRANSACTION_TYPE.REFERRAL_REWARD, description: '推荐奖励（被推荐人）' });
  await db.update(referralRelationships).set({ rewardGranted: true }).where(eq(referralRelationships.id, relation.id));

  // Optional: mark an achievement flag so we can prevent reprocessing by other systems
  const existsAch = await db
    .select({ id: achievements.id })
    .from(achievements)
    .where(and(eq(achievements.userId, userId), eq(achievements.achievementId, 'activation_rewarded')))
    .limit(1);
  if (existsAch.length === 0) {
    await db.insert(achievements).values({ userId, achievementId: 'activation_rewarded', achievementName: 'activation_rewarded', rewardAmount: 0 });
  }
...
export async function recordChatRoundAndTryActivate(userId: string) {
  await incrementTaskProgress({ userId, taskId: 'ai_chat_rounds', target: 3, delta: 1 });
