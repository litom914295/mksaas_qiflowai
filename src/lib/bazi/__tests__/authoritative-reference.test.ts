...
describe('权威干支数据验证', () => {
  ...
  it('应该验证所有权威数据的准确性', () => {
    ...
    for (const [dateStr, result] of Object.entries(results)) {
      console.log(
        `${dateStr}: 期望=${result.expected}, 计算=${result.calculated}, 匹配=${result.match ? '✓' : '✗'}`
      );
...
describe('甲子周期验证', () => {
  it('应该在60天后回到相同的日柱', () => {
    ...
    console.log(
      `基准日期: ${baseDate.toISOString().slice(0, 10)} => ${basePillar}`
    );
    console.log(
      `60天后: ${cycleDate.toISOString().slice(0, 10)} => ${cyclePillar}`
    );
  ...
  it('应该测试多个甲子周期', () => {
    ...
    [1, 2, 3, 5, 10].forEach((cycles) => {
      ...
      console.log(
        `${cycles}甲子后 (${cycles * 60}天): ${testDate.toISOString().slice(0, 10)} => ${testPillar}`
      );
