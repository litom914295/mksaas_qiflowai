...
const BRANCH_HIDDEN_STEMS: Record<
  Branch,
  Array<{ stem: Stem; strength: number }>
> = {
...
// 重要的十神组合配置
const IMPORTANT_COMBINATIONS: Record<
  TenGodCombination,
  {
    gods: TenGod[];
    conditions: string[];
 ...
 * 计算十神位置和力量
 */
function analyzeTenGodPositions(
  pillars: Pillars
): Map<TenGod, TenGodPosition[]> {
...
function calculateTenGodStrength(positions: TenGodPosition[]): {
  ...
  const totalStrength = positions.reduce((sum, pos) => {
    ...
    const positionWeight =
      pos.pillar === 'month' ? 1.2 : pos.pillar === 'day' ? 1.1 : 1.0;
...
function analyzeTenGodCombinations(
  ...
  for (const [combName, config] of Object.entries(IMPORTANT_COMBINATIONS)) {
    ...
    // 检查所需十神是否都存在且有一定力量
    const presentGods = tenGodsInfo.filter(
      (info) =>
    ...
    if (presentGods.length === requiredGods.length) {
      // 计算组合强度
      const avgStrength =
        presentGods.reduce((sum, info) => sum + info.totalStrength, 0) /
        presentGods.length;

      // 检查成立条件
      const metConditions = checkCombinationConditions(
        combination,
        pillars,
        presentGods
      );
      ...
      if (metConditions >= 0.8) {
        effect =
          config.positive_effects.length > config.negative_effects.length
            ? 'very_positive'
            : 'positive';
...
function checkCombinationConditions(
  ...
  switch (combination) {
    case '官印相生': {
      // 正官正印配合，印在月令更佳
      const hasMonthPrint = presentGods.some(
        (god) =>
          god.god === '正印' &&
          god.positions.some((pos) => pos.pillar === 'month')
      );
      const hasStrongOfficial = presentGods.some(
        (god) => god.god === '正官' && god.influence !== 'weak'
      );
    ...
    case '财官相生': {
      // 财星生官星，财官都要有力
      const strongWealth = presentGods.find(
        (god) => god.god === '正财'
      )?.influence;
      const strongOfficial2 = presentGods.find(
        (god) => god.god === '正官'
      )?.influence;
      if (strongWealth && ['strong', 'very_strong'].includes(strongWealth))
        score += 0.2;
      if (
        strongOfficial2 &&
        ['strong', 'very_strong'].includes(strongOfficial2)
      )
        score += 0.2;
      break;
    }
    ...
    default: {
      // 通用评判：看十神力量和配置
      const avgStrength =
        presentGods.reduce((sum, god) => {
          const strengthValue =
            god.influence === 'very_strong'
              ? 5
              : god.influence === 'strong'
                ? 4
                : god.influence === 'medium'
                  ? 3
                  : god.influence === 'weak'
                    ? 2
                    : 1;
 ...
 * 分析柱位影响
 */
function analyzePillarInfluence(
  pillars: Pillars
): EnhancedTenGodsAnalysis['pillarAnalysis'] {
  const dayMaster = pillars.day.stem;
 ...
 * 主函数：增强型十神分析
 */
export function enhancedTenGodsAnalysis(
  pillars: Pillars
): EnhancedTenGodsAnalysis {
  // 分析十神位置和力量
  ...
  for (const info of tenGodsInfo) {
    const current = categoryStats.get(info.category) || {
      count: 0,
      strength: 0,
    };
  ...
  const totalStrength = tenGodsInfo.reduce(
    (sum, info) => sum + info.totalStrength,
    0
  );
  const distribution = Array.from(categoryStats.entries()).map(
    ([category, stats]) => ({
      category,
      count: stats.count,
  ...
  const balance = Math.min(
    100,
    100 - (Math.max(...distribution.map((d) => d.dominance)) - 20)
  );
  const potential = tenGodsInfo.reduce((sum, info) => {
    const bonus =
      info.influence === 'very_strong'
        ? 15
        : info.influence === 'strong'
          ? 10
          : info.influence === 'medium'
            ? 5
            : 0;
    return sum + bonus;
  }, 50);
