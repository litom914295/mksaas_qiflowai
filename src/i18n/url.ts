 ...
 * 3) 兜底: 'zh-CN'（与业务目标一致）
 */
export function getPreferredLocaleFromRequest(
  req: NextRequest,
  fallback = 'zh-CN'
): string {
  try {
    ...
    if (
      cookie &&
      LOCALES.some((l) => l.toLowerCase() === cookie.toLowerCase())
    ) {
    ...
    if (al) {
      ...
      for (const lang of items) {
        ...
        if (lang.startsWith('zh')) {
          const zh =
            LOCALES.find((l) => l.toLowerCase() === 'zh-cn') ||
            LOCALES.find((l) => l.toLowerCase().startsWith('zh'));
          if (zh) return zh;
        }
