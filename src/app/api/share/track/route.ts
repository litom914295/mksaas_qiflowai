  ...
  try {
    ...
    const [rec] = await db
      .select({
        ...
        clickCount: shareRecords.clickCount,
        conversionCount: shareRecords.conversionCount,
        rewardGranted: shareRecords.rewardGranted,
    ...
    try {
      await db
        .insert(shareClicks)
        .values({ shareId: id as any, ip, userAgent: ua, fingerprint: fp });
    } catch {}
    ...
    if ((black?.length || 0) > 0 || (black2?.length || 0) > 0) {
      ...
      return NextResponse.json({
        success: true,
        data: { updated: 'convert', rewarded: false, reason: 'BLACKLISTED' },
      });
    }
    ...
    if (
      !rec.rewardGranted &&
      (websiteConfig.growth.share.requireConvert
        ? (rec.conversionCount || 0) + 1 >= 1
        : true)
    ) {
      const now = new Date();
      ...
      const todays = await db
        ...
        .where(
          and(
            eq(shareRecords.userId, rec.userId),
      ...
      if ((todays?.length || 0) >= dailyMax) {
        return NextResponse.json({
          success: true,
          data: { updated: 'convert', rewarded: false, reason: 'DAILY_LIMIT' },
        });
      }
      ...
      if (cooldownMin > 0) {
        ...
        const lastWithin = await db
          ...
          .where(
            and(
              eq(shareRecords.userId, rec.userId),
              eq(shareRecords.rewardGranted, true),
              gte(shareRecords.createdAt as any, since as any)
            )
          )
          .limit(1);
        if (lastWithin && lastWithin.length > 0) {
          return NextResponse.json({
            success: true,
            data: { updated: 'convert', rewarded: false, reason: 'COOLDOWN' },
          });
        }
      }
      ...
      if (credits > 0) {
        await addCredits({
          ...
          description: '分享奖励（有效转化）',
        });
      }
      await db
        .update(shareRecords)
        .set({ rewardGranted: true, rewardAmount: credits })
        .where(eq(shareRecords.id, id));

      return NextResponse.json({
        success: true,
        data: { updated: 'convert', rewarded: true, credits },
      });
    }

    return NextResponse.json({
      success: true,
      data: { updated: 'convert', rewarded: false },
    });
  } catch (error) {
