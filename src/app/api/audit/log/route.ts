...
export async function GET(request: NextRequest) {
  ...
  const stats = {
    ...
    sensitiveFiltered: filteredLogs.filter(
      (l) => l.responseType === 'SENSITIVE_FILTER'
    ).length,
    guidanceProvided: filteredLogs.filter((l) => l.responseType === 'GUIDANCE')
      .length,
    analysisCompleted: filteredLogs.filter((l) => l.responseType === 'ANALYSIS')
      .length,
...
function checkAnomalies(log: AuditLog) {
  ...
  if (recentSensitiveLogs.length > 5) {
    console.warn(
      '[ANOMALY DETECTED] High frequency of sensitive topic attempts:',
      {
  ...
  const errorRate =
    sessionLogs.filter((l) => l.responseType === 'ERROR').length /
    sessionLogs.length;
