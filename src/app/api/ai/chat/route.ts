...
function parseUserInput(text: string) {
  ...
  const time = timeMatch ? `${timeMatch[1].padStart(2, '0')}:${timeMatch[2].padStart(2, '0')}` : null;
...
interface ChatResponse {
  ...
  data?: {
    ...
    sessionId: string;
    confidence?: number;
    birthInfo?: { date: string; time: string | null; gender: string | null; hasComplete: boolean };
...
function generateSmartResponse(message: string, baziData: any): string {
  ...
  // 用神相关问题
  if (message.includes('用神') || message.includes('喜') || message.includes('五行')) {
    ...
    response += `💡 **生活建议：**\n`;
    response += yongShen?.suggestions || `建议您多接触${yongShen?.favorable?.join('、')}属性的事物，避免${yongShen?.unfavorable?.join('、')}属性的环境。`;
    
    return response;
  }
  
  // 财运相关问题
  if (message.includes('财') || message.includes('钱') || message.includes('收入')) {
  ...
  // 感情婚姻问题
  if (message.includes('感情') || message.includes('婚姻') || message.includes('爱情') || message.includes('对象')) {
    let response = `**感情婚姻分析：**\n\n`;
  ...
  // 健康相关问题
  if (message.includes('健康') || message.includes('身体') || message.includes('疾病')) {
 ...
 * 调用AI模型
 */
async function callAIModel(prompt: string, systemPrompt?: string, baziData?: any): Promise<string> {
  ...
  if (GEMINI_API_KEY) {
    try {
      ...
      return data.candidates?.[0]?.content?.parts?.[0]?.text || '抱歉，我暂时无法回答您的问题。';
    } catch (error) {
 ...
 */
function generateSystemPrompt(questionType: QuestionType, hasData: boolean, baziData?: any): string {
...
### 五行分析
${baziData.fiveElements ? Object.entries(baziData.fiveElements).map(([k, v]) => `- ${k}: ${v}`).join('\n') : ''}
    ...
    if (birthInfoToUse && birthInfoToUse.hasComplete) {
      ...
      if (isNewParse || !baziResult) {
        ...
        try {
          ...
          // 调试：查看实际返回的数据结构
          console.log('🔍 [DEBUG] 原始数据结构 pillars:', JSON.stringify(rawBaziResult?.pillars, null, 2));
      ...
      const hasQuestion = message.includes('？') || message.includes('?') || 
                         ...
                         message.includes('怎么') || message.includes('用神') ||
                         message.includes('五行') || message.includes('喜') ||
                         message.includes('财运') || message.includes('财富') ||
                         message.includes('运势') || message.includes('运程') ||
                         message.includes('今年') || message.includes('明年') ||
                         message.includes('事业') || message.includes('健康') ||
                         message.includes('感情') || message.includes('婚姻') ||
                         message.includes('工作') || message.includes('学业');
      ...
      if (baziResult && (hasQuestion || !isNewParse)) {
        ...
        try {
          ...
          const aiResponse = await callAIModel(userPrompt, systemPrompt, baziResult);
      ...
      return NextResponse.json<ChatResponse>({
        success: true,
        data: {
          response: `🔍 我识别到您的出生日期是 **${parsedBirth.date}**\n\n` +
            `但还需要以下信息才能进行完整分析：\n` +
...
请基于这些信息和八字命理的通用知识回答用户的问题。`;
    ...
    const systemPrompt = generateSystemPrompt(questionType, validation.hasData || !!birthInfoToUse);
