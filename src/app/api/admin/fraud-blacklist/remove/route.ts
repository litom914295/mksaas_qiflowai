...
export async function POST(request: Request) {
  try {
    ...
    if (!id && !ip && !fingerprint) {
      return NextResponse.json(
        { success: false, error: 'MISSING_KEY' },
        { status: 400 }
      );
    }
    ...
    if (id) {
      ...
      if (ip)
        await db.delete(fraudBlacklist).where(eq(fraudBlacklist.ip, ip as any));
      if (fingerprint)
        await db
          .delete(fraudBlacklist)
          .where(eq(fraudBlacklist.fingerprint, fingerprint as any));
    }
