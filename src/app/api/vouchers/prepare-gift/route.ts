...
export async function POST(request: Request) {
  ...
  if (!voucherId) return NextResponse.json({ success: false, error: 'INVALID_VOUCHER_ID' }, { status: 400 });
  ...
  try {
    ...
    return NextResponse.json({ success: false, error: e?.message || 'GIFT_PREPARE_FAILED' }, { status: 400 });
  }
}