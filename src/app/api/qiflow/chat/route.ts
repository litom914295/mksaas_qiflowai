...
import { getSystemPrompt, AI_FENGSHUI_QUICK_PROMPT } from '@/lib/qiflow/ai/system-prompt';
...
function detectQuestionType(message: string) {
  const baziKeywords = ['八字', '命理', '五行', '用神', '喜神', '忌神', '大运', '流年', 
                        '财运', '事业', '婚姻', '感情', '健康', '性格', '天干', '地支', 
                        '十神', '正财', '偏财', '正官', '七杀', '正印', '偏印', '食神', '伤官'];
  const fengshuiKeywords = ['风水', '玄空', '飞星', '九宫', '朝向', '坐向', '山星', '水星', 
                           '财位', '文昌', '桃花位', '煞位', '房屋', '住宅', '办公室', '房子',
                           '布置', '布局', '摆放', '装修', '家居', '卧室', '客厅', '厨房'];
...
async function generateAIResponse(
  ...
  // 使用优化后的系统提示词
  const systemPrompt = getSystemPrompt({ hasUserBazi: true }) + 
    `\n\n【当前用户八字数据】\n` +
    `四柱八字：\n` +
    ...
    (baziData.universalFortune ? `大运：${JSON.stringify(baziData.universalFortune)}\n` : '') +
    (baziData.yearlyFortune ? `流年运势：${JSON.stringify(baziData.yearlyFortune)}` : '');

  try {
    // 尝试使用配置的AI服务
    const models = [
      { provider: 'deepseek', model: () => openai('deepseek-chat'), key: process.env.DEEPSEEK_API_KEY },
      { provider: 'openai', model: () => openai('gpt-3.5-turbo'), key: process.env.OPENAI_API_KEY },
      { provider: 'google', model: () => google('gemini-pro'), key: process.env.GOOGLE_API_KEY },
    ];
...
function generatePersonalizedFengshuiAnalysis(houseInfo: any, baziData?: any) {
  ...
  if (!degree && houseInfo.facing) {
    const directionMap: any = {
      '北': 0, '东北': 45, '东': 90, '东南': 135,
      '南': 180, '西南': 225, '西': 270, '西北': 315,
      '坐北朝南': 180, '坐南朝北': 0, '坐东朝西': 270, '坐西朝东': 90,
      '坐东南朝西北': 315, '坐西北朝东南': 135,
      '坐东北朝西南': 225, '坐西南朝东北': 45,
    };
  ...
  const getMountainFacing = (deg: number) => {
    if (deg >= 337.5 || deg < 22.5) return { mountain: '子', facing: '午', palace: '坎宫' };
    if (deg >= 22.5 && deg < 67.5) return { mountain: '丑寅', facing: '未申', palace: '艮宫' };
    if (deg >= 67.5 && deg < 112.5) return { mountain: '卯甲', facing: '酉庚', palace: '震宫' };
    if (deg >= 112.5 && deg < 157.5) return { mountain: '乙辰', facing: '辛戌', palace: '巽宫' };
    if (deg >= 157.5 && deg < 202.5) return { mountain: '巳丙', facing: '亥壬', palace: '离宫' };
    if (deg >= 202.5 && deg < 247.5) return { mountain: '丁未', facing: '癸丑', palace: '坤宫' };
    if (deg >= 247.5 && deg < 292.5) return { mountain: '申庚', facing: '寅甲', palace: '兑宫' };
    if (deg >= 292.5 && deg < 337.5) return { mountain: '辛戌', facing: '乙辰', palace: '乾宫' };
  ...
  if (baziData) {
    ...
    // 分析五行强弱
    const elementStrength = Object.entries(fiveElements)
      .sort((a, b) => (b[1] as number) - (a[1] as number));
    const strongestElement = elementStrength[0]?.[0] || '水';
    const weakestElement = elementStrength[elementStrength.length - 1]?.[0] || '火';
...
function getDirectionByElement(element: string) {
  const elementDirectionMap: any = {
    ...
    '土': { direction: '中央、东北、西南', element: '土', description: '稳定之气' },
...
// 生成风水AI回答
async function generateFengshuiResponse(message: string, fengshuiData: any): Promise<string> {
  // 使用优化后的系统提示词
  const systemPrompt = getSystemPrompt({ hasHouseInfo: true }) + 
    `\n\n【当前房屋风水数据】\n` +
    ...
    `九宫飞星分布：\n` +
    Object.entries(fengshuiData.nineStars).map(([k, v]: [string, any]) => 
...
async function generateCombinedFengshuiResponse(
  ...
  const systemPrompt = getSystemPrompt({ hasUserBazi: true, hasHouseInfo: true }) + 
    `\n\n【综合分析数据】\n` +
    `\n一、用户八字信息：\n` +
    `日主：${baziData.dayMaster}\n` +
    `五行分析：${JSON.stringify(baziData.fiveElements)}\n` +
  ...
  try {
    ...
    // 检查用户关注点
    const isWealthFocused = message.includes('财运') || message.includes('旺财') || message.includes('财位');
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (isBaziQuestion) {
      ...
      // 没有生辰信息，要求用户提供
      return NextResponse.json({
        response: '为了准确分析您的八字命理，我需要您的出生信息：\n\n' +
                 '请提供：\n' +
                 '1. 出生年月日（如：1990年1月1日）\n' +
                 '2. 出生时间（如：下午3点30分）\n' +
                 '3. 性别（男/女）\n' +
    ...
    if (parsedBirthInfo && !isBaziQuestion) {
      ...
      try {
        ...
        // 生成基础八字信息
        const basicInfo = `已识别您的生辰信息并完成八字排盘：\n\n` +
    ...
    if (isFengshuiQuestion) {
      // 【核心原则】风水分析必须基于八字命理
      // 先检查是否有八字数据
      let userBaziData = context?.calculatedBazi;
      
      // 6.1 如果没有八字数据，要求先提供
      if (!userBaziData) {
        return NextResponse.json({
          response: '要为您提供**个性化的风水布局方案**，我需要先了解您的八字信息。这样才能根据您的五行喜忌，确定最适合您的财位、文昌位等方位。\n\n' +
                   '请告诉我：\n' +
                   '📅 您的出生年月日时（请注明公历或农历）\n' +
                   '👤 性别\n' +
                   '📍 出生地点（用于时区校正）\n\n' +
      ...
      if (houseInfo && userBaziData && !calculatedFengshui) {
        try {
          // 【核心】基于用户八字的个性化风水计算
          calculatedFengshui = generatePersonalizedFengshuiAnalysis(houseInfo, userBaziData);
        } catch (error) {
      ...
      if (calculatedFengshui && userBaziData) {
        // 必须结合八字分析
        const combinedAnalysis = await generateCombinedFengshuiResponse(message, calculatedFengshui, userBaziData);
    ...
    if (parsedHouseInfo && !isFengshuiQuestion && !isBaziQuestion) {
      // 用户提供了房屋信息，但没有具体问题
      try {
        const calculatedFengshui = generatePersonalizedFengshuiAnalysis(parsedHouseInfo);
        
        // 生成基础风水信息
        const basicInfo = `已识别您的房屋信息并完成风水分析：\n\n` +
          `🏠 房屋朝向：${parsedHouseInfo.facing || parsedHouseInfo.degree + '度'}\n` +
          (parsedHouseInfo.buildYear ? `📅 建造年份：${parsedHouseInfo.buildYear}年\n` : '') +
          `\n**九宫飞星分析**\n` +
