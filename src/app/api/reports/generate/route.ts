...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const sessionId =
      req.headers.get('x-session-id') || `session_${Date.now()}`;

    const isPremium = Boolean(input.houseInfo);
    const reportType = isPremium ? 'essential' : 'basic';

    // 1. 成本检查（4层防护）
    const estimatedCost = isPremium ? 0.5 : 0.02;
    ...
    // 2. 生成报告
    console.log(
      `[Report API] Generating ${reportType} report for user ${userId}`
    );
...
export async function GET() {
  ...
  return NextResponse.json({
    ...
    canGenerate: {
      ...
      essential: globalCostGuard.canExecute(0.5, 'anonymous').allowed,
    },
  });
}
