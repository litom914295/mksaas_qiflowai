...
export async function processReferralRegistration({
  ...
  try {
    ...
    const todayCount = todayReferrals.rows[0]?.count || 0;
    ...
    return {
      success: true,
      referrerReward: todayCount < referralConfig.directReferral.maxDaily ? referralConfig.directReferral.referrer : 0,
 ...
 * 处理二级推荐奖励
 */
async function processIndirectReferralReward(directReferrerId: string, newUserId: string) {
  const db = await getDb();
  ...
  if (result.rows && result.rows.length > 0) {
    const indirectReferrerId = result.rows[0].referrer_id;
...
async function checkAndGrantMilestoneRewards(userId: string) {
  ...
  if (!stats.rows || stats.rows.length === 0) return;
  
  const directReferrals = stats.rows[0].direct_referrals;
  const currentLevel = stats.rows[0].referral_level;
  ...
  for (const milestone of referralConfig.milestones) {
    if (directReferrals >= milestone.count) {
      ...
      if (!achieved.rows || achieved.rows.length === 0) {
        // 发放里程碑奖励
        await addCredits({
          userId,
...
export async function recordShare({
  ...
  const todayCount = todayShares.rows[0]?.count || 0;
  ...
  if (todayCount < referralConfig.shareRewards.maxDailyShares) {
    const reward = referralConfig.shareRewards[shareType as keyof typeof referralConfig.shareRewards] || 5;
