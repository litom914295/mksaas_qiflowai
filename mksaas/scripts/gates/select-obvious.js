...
let data;
try { data = JSON.parse(fs.readFileSync(cpath, 'utf8').replace(/^\uFEFF/, '')); } catch (e) {
...
const selected = files.filter(f => patterns.some(re => re.test(f.replace(/\\/g,'/'))));
const out = { files: selected, counts: { total: selected.length } };
fs.writeFileSync(path.join(ART, 'candidates.obvious.json'), JSON.stringify(out, null, 2));
