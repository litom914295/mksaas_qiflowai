...
if (tasksData.tags.master) {
  ...
  let totalTasks = masterTasks.length;
  ...
  masterTasks.forEach(task => {
    ...
    if (task.subtasks && task.subtasks.length > 0) {
      ...
      task.subtasks.forEach(subtask => {
        ...
        console.log(`    ${subtaskStatus} ${subtask.id}. ${subtask.title} (${subtask.status})`);
        if (subtask.status === 'done') completedSubtasks++;
      });
      console.log(`    📊 子任务完成率: ${completedSubtasks}/${task.subtasks.length}`);
    }
  });
...
if (tasksData.tags['migration-qiflow']) {
  ...
  let totalMigrationTasks = migrationTasks.length;
