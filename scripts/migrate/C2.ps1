# C2: 依赖与配置对齐（最小侵入）
# 运行位置：项目根目录
# 产物：mksaas/artifacts/C2/{deps.diff,typecheck.txt,install.log}
...
Ensure-Dir "mksaas/artifacts/C2"
...
if (Test-Path "qiflow-ai/package.json") {
  try {
    ...
    $diff | Set-Content -Encoding UTF8 "mksaas/artifacts/C2/deps.diff"
  } catch {
    "git diff failed (ignored): $($_.Exception.Message)" | Set-Content -Encoding UTF8 "mksaas/artifacts/C2/deps.diff"
  }
} else {
  "source qiflow-ai/package.json not found, skip diff" |
    Set-Content -Encoding UTF8 "mksaas/artifacts/C2/deps.diff"
}
...
# 安装依赖并记录日志
$installLog = "mksaas/artifacts/C2/install.log"
try {
...
$typeLog = "mksaas/artifacts/C2/typecheck.txt"
try {
...
Write-Host "[C2] artifacts created at mksaas/artifacts/C2"
