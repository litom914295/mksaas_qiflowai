...
# 运行位置：项目根目录
# 产物：mksaas/artifacts/C1/{source_files.txt,target_dirs.txt,mapping.md}
...
Ensure-Dir "mksaas/artifacts/C1"
...
if (Test-Path "qiflow-ai") {
  Get-ChildItem -Recurse -File -Path "qiflow-ai" |
    ...
    Set-Content -Encoding UTF8 "mksaas/artifacts/C1/source_files.txt"
} else {
  "" | Set-Content -Encoding UTF8 "mksaas/artifacts/C1/source_files.txt"
}
...
if (Test-Path "src") {
  Get-ChildItem -Recurse -Directory -Path "src" |
    Sort-Object FullName |
    Select-Object -ExpandProperty FullName |
    Set-Content -Encoding UTF8 "mksaas/artifacts/C1/target_dirs.txt"
} else {
  "" | Set-Content -Encoding UTF8 "mksaas/artifacts/C1/target_dirs.txt"
}
...
$mapping | Set-Content -Encoding UTF8 "mksaas/artifacts/C1/mapping.md"

Write-Host "[C1] artifacts created at mksaas/artifacts/C1"
