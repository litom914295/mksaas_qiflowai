...
async function testEndpoint(config) {
  ...
  log(
    `限流策略: ${config.rateLimit.requests} 次/${config.rateLimit.window}`,
    'cyan'
  );
  log('='.repeat(60), 'cyan');
  ...
  for (let i = 1; i <= maxAttempts; i++) {
    ...
    try {
      ...
      if (response.status === 429) {
        ...
        results.successful++;
        log(
          `  请求 #${i}: ✅ 成功 (${response.status}) - ${responseTime}ms`,
          'green'
        );
      } else {
        results.errors++;
        log(
          `  请求 #${i}: ⚠️ 错误 (${response.status}) - ${responseTime}ms`,
          'red'
        );
      }
    } catch (error) {
  ...
  log(
    ...
    results.successful > 0 ? 'green' : 'red'
  );
  log(
    `  被限流: ${results.rateLimited}`,
    results.rateLimited > 0 ? 'green' : 'yellow'
  );
  ...
  const isWorkingCorrectly =
    results.successful <= expectedSuccessful && results.rateLimited > 0;

  log('\n🎯 限流验证:', 'blue');
  if (isWorkingCorrectly) {
    log('  ✅ 限流正常工作！', 'green');
    log(
      `  - 成功请求数 (${results.successful}) ≤ 限制 (${config.rateLimit.requests})`,
      'green'
    );
    log('  - 正确触发了限流响应', 'green');
  } else {
    ...
    if (results.successful > expectedSuccessful) {
      log(
        `  - 成功请求数 (${results.successful}) 超过了限制 (${config.rateLimit.requests})`,
        'yellow'
      );
    }
