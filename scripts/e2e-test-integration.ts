...
class E2EIntegrationTester {
   ...
   */
  async runTests() {
    console.log(`${colors.bright}${colors.blue}🚀 开始端到端集成测试...${colors.reset}\n`);
    console.log(`${colors.yellow}注意：请确保开发服务器正在运行 (npm run dev)${colors.reset}\n`);
   ...
   * 运行单个测试用例
   */
  async runTestCase(testCase: TestCase): Promise<{ passed: boolean; reason?: string }> {
    ...
    try {
      ...
      if (testCase.expectedBehavior.requiresBazi) {
        ...
        if (response.includes('八字') || response.includes('出生信息')) {
          console.log(`${colors.green}✅ PASS：正确要求提供八字信息${colors.reset}`);
          return { passed: true };
        } else {
      ...
      if (testCase.expectedBehavior.providesPersonalized) {
        // 应该提供个性化建议
        const hasPersonalized = this.checkPersonalization(response, testCase.request.context);
        if (hasPersonalized) {
          console.log(`${colors.green}✅ PASS：提供了个性化建议${colors.reset}`);
          return { passed: true };
        } else {
  ...
  async simulateAPICall(request: any): Promise<string> {
    ...
    // 检查是否包含风水相关问题
    const isFengShuiQuestion = message.includes('财位') || 
                               message.includes('风水') || 
                               message.includes('布局') ||
                               message.includes('颜色');
  ...
  generateReport(passCount: number, failCount: number) {
    ...
    const rateColor = passRate >= 80 ? colors.green : 
                     passRate >= 60 ? colors.yellow : colors.red;
    ...
    if (passRate >= 80) {
      console.log(`${colors.green}✅ 核心原则"风水必须基于八字"已彻底贯彻！${colors.reset}`);
      console.log('\n改进建议：');
      console.log('1. 💡 继续优化个性化算法精度');
      console.log('2. 💡 增加更多基于八字的细分功能');
      console.log('3. 💡 建立用户反馈机制验证效果');
    } else if (passRate >= 60) {
      console.log(`${colors.yellow}⚠️ 核心原则基本贯彻，但仍有改进空间${colors.reset}`);
      console.log('\n必要改进：');
      console.log('1. 🔴 加强风水分析的八字前置验证');
      console.log('2. 🔴 确保所有风水建议都个性化');
      console.log('3. 🟡 完善错误提示和用户引导');
    } else {
      console.log(`${colors.red}❌ 核心原则未充分贯彻，需要重大改进${colors.reset}`);
      console.log('\n紧急修复：');
