...
async function runTest(testCase, previousResult = null) {
  ...
  try {
    ...
    if (expected.type && result.type !== expected.type) {
      console.error(`❌ 类型不匹配: 期望 ${expected.type}, 实际 ${result.type}`);
      passed = false;
    }
    ...
    if (expected.creditsUsed !== undefined && result.creditsUsed !== expected.creditsUsed) {
      console.error(`❌ 积分消耗不匹配: 期望 ${expected.creditsUsed}, 实际 ${result.creditsUsed}`);
      passed = false;
    }
