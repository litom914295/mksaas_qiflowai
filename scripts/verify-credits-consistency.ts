...
import { baziCalculations, fengshuiAnalysis, userCredit, creditTransaction } from '../src/db/schema'
...
async function verifyCreditsConsistency() {
	...
	try {
		...
		for (const user of users.slice(0, 3)) {
			...
			console.log(`     八字计算消耗: ${baziCreditsUsed} (${baziRecords.length} 次)`)
			...
			console.log(`     玄空风水消耗: ${xuankongCreditsUsed} (${xuankongRecords.length} 次)`)
		...
		if (creditFlow[0]) {
			...
			console.log(`  净流入: ${Number(creditFlow[0].totalIn) - Number(creditFlow[0].totalOut)}`)
		}
