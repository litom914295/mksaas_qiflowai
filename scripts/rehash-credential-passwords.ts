...
async function scryptHash(password: string) {
  ...
  const key = await scryptAsync(password.normalize('NFKC'), saltHex, { N, r, p, dkLen, maxmem: 128 * N * r * 2 });
...
async function rehash(email: string, plain: string) {
  ...
  const u = users[0];
  const accs = await db.select().from(account).where(and(eq(account.userId, u.id), eq(account.providerId, 'credential'))).limit(1);
  if (accs.length === 0) {
