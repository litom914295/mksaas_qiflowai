...
class BaziFengShuiIntegrationTester {
   ...
   */
  async runAllTests() {
    console.log(`${colors.bright}${colors.blue}🔍 开始检查"风水必须基于八字"原则的贯彻情况...${colors.reset}\n`);
  ...
  async testSystemPrompts() {
    ...
    if (fs.existsSync(promptFile)) {
      ...
      // 检查核心原则是否存在
      const hasCorePrinciple = content.includes('八字定制风水原则') || 
      ...
      // 检查是否强调个性化
      const hasPersonalization = content.includes('个性化') && content.includes('定制');
      ...
      this.results.push({
        ...
        suggestion: hasImplementationDetails < 4 
  ...
  async testAPIRoutes() {
    ...
    if (fs.existsSync(apiFile)) {
      ...
      // 检查风水请求时是否验证八字
      const hasBaziCheck = content.includes('!baziData') && 
                          content.includes('风水分析必须基于');
      ...
      // 检查错误提示
      const hasUserFriendlyError = content.includes('请先提供') || 
                                   content.includes('需要您的出生信息');
  ...
  async testFrontendComponents() {
    ...
    const chatInterface = path.join(this.rootDir, 'src/components/qiflow/ai/ai-chat-interface.tsx');
    
    if (fs.existsSync(chatInterface)) {
      const content = fs.readFileSync(chatInterface, 'utf-8');
      
      // 检查开场白是否说明核心优势
      const hasIntroduction = content.includes('核心优势') || 
                            content.includes('所有风水分析都基于');
      ...
      // 检查快捷问题引导
      const hasGuidedQuestions = content.includes('基于我的八字') || 
                                content.includes('命理财位');
  ...
  async testServiceIntegration() {
    ...
    if (fs.existsSync(integrationFile)) {
      ...
      this.results.push({
        category: '服务整合',
        test: '个性化计算完整性',
        status: hasFunctions === personalizationFunctions.length ? 'PASS' : 'WARNING',
        details: `实现了 ${hasFunctions}/${personalizationFunctions.length} 个个性化计算函数`,
        suggestion: hasFunctions < personalizationFunctions.length 
          ? '建议完善所有个性化计算函数'
          : undefined
      });
    } else {
  ...
  async testDocumentation() {
    ...
    filesToCheck.forEach(file => {
      ...
      if (fs.existsSync(fullPath)) {
        ...
        // 检查是否有说明核心原则的注释
        const hasPrincipleComment = comments.some(comment => 
          comment.includes('八字') && comment.includes('风水')
        );
    ...
    this.results.push({
      ...
      suggestion: principleComments < 2 
        ? '建议在更多关键文件中添加原则说明注释'
        : undefined
    });
  }
  ...
  generateReport() {
    ...
    categories.forEach(category => {
      ...
      categoryResults.forEach(result => {
        ...
        console.log(`  ${statusIcon} ${result.test}: ${statusColor}${result.status}${colors.reset}`);
    ...
    console.log(`   通过: ${colors.green}${stats.passed}${colors.reset} | 失败: ${colors.red}${stats.failed}${colors.reset} | 警告: ${colors.yellow}${stats.warnings}${colors.reset}`);
  ...
  saveJSONReport(stats: any, score: number) {
    ...
    const reportPath = path.join(this.rootDir, 'test-results-bazi-fengshui.json');
