  ...
  try {
    ...
    if (luckPillars) {
      ...
      luckPillars.slice(0, 5).forEach((p, i) => {
        console.log(`  ${i+1}. ${p.heavenlyStem}${p.earthlyBranch}  ${p.startAge}-${p.endAge}岁  period=${p.period}`);
      });
    }
    ...
    normalized.luck.daYunTimeline.slice(0, 5).forEach((d, i) => {
      console.log(`  ${i+1}. ${d.heavenlyStem}${d.earthlyBranch}  ${d.ageRange[0]}-${d.ageRange[1]}岁  (${d.yearRange[0]}-${d.yearRange[1]}年)  period=${d.period}`);
    });

    console.log('\n当前大运:');
    if (normalized.luck.currentDaYun) {
      const curr = normalized.luck.currentDaYun;
      console.log(`  ${curr.heavenlyStem}${curr.earthlyBranch}  ${curr.ageRange[0]}-${curr.ageRange[1]}岁  (${curr.yearRange[0]}-${curr.yearRange[1]}年)`);
    } else {
    ...
    if (shouldMatch) {
      console.log('✅ 应该匹配的大运:', `${shouldMatch.heavenlyStem}${shouldMatch.earthlyBranch} ${shouldMatch.ageRange[0]}-${shouldMatch.ageRange[1]}岁`);
      if (normalized.luck.currentDaYun) {
        const curr = normalized.luck.currentDaYun;
        if (curr.period === shouldMatch.period) {
          ...
          console.log('   期望:', `${shouldMatch.heavenlyStem}${shouldMatch.earthlyBranch}`);
      ...
      console.log('ℹ️  当前年龄', currentAge, '岁，尚未起运或不在任何大运范围内');
    }
