...
const checkSecureContext = () => {
  ...
  console.log(`1. 安全上下文: ${isSecure ? '✅ 安全（HTTPS/localhost）' : '❌ 不安全'}`);
  return isSecure;
};
...
const checkServiceWorkerRegistration = async () => {
  ...
  try {
    ...
    if (registrations.length > 0) {
      console.log(`3. Service Worker 注册: ✅ 已注册 ${registrations.length} 个`);
...
const checkInstallPrompt = () => {
  ...
  window.addEventListener('beforeinstallprompt', (e) => {
    ...
    console.log('   💡 提示: 可以调用 window.showInstallPrompt() 来显示安装对话框');
    ...
    window.showInstallPrompt = async () => {
      if (deferredPrompt) {
        ...
        console.log(`   用户选择: ${outcome === 'accepted' ? '✅ 已安装' : '❌ 已拒绝'}`);
        deferredPrompt = null;
      } else {
...
const runAllChecks = async () => {
  ...
  console.log('- 在 Chrome DevTools > Application > Service Workers 查看注册状态');
  console.log('- 使用 Chrome://flags/#enable-desktop-pwas 启用桌面 PWA（如需要）');
};
