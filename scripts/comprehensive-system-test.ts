  ...
  async testCorePrinciples(): Promise<TestResult[]> {
    ...
    if (fs.existsSync(apiFile)) {
      ...
      results.push({
        ...
        subcategory: 'API实现',
        test: '风水前置八字验证',
        status: content.includes('isFengShuiQuestion && !hasBaziData')
          ? 'PASS'
          : 'FAIL',
    ...
    for (const route of apiRoutes) {
      ...
      if (exists) {
        ...
        results.push({
          ...
          status:
            content.includes('z.object') || content.includes('zod')
              ? 'PASS'
              : 'WARNING',
          details: '使用Zod进行输入验证',
          severity: 'MEDIUM',
  ...
  printCategoryResults(results: TestResult[]) {
    ...
    for (const [subcategory, tests] of Object.entries(grouped)) {
      ...
      for (const test of tests) {
        ...
        console.log(
          `    ${icon} ${test.test}: ${statusColor}${test.status}${colors.reset}`
        );
...
${colors.reset}`);
    ...
    console.log(
      `✅ 通过: ${colors.green}${stats.passed}${colors.reset} (${((stats.passed / stats.total) * 100).toFixed(1)}%)`
    );
    console.log(
      `❌ 失败: ${colors.red}${stats.failed}${colors.reset} (${((stats.failed / stats.total) * 100).toFixed(1)}%)`
    );
    console.log(
      `⚠️  警告: ${colors.yellow}${stats.warnings}${colors.reset} (${((stats.warnings / stats.total) * 100).toFixed(1)}%)`
    );
    ...
    if (bySeverity.critical > 0 || bySeverity.high > 0) {
      ...
      if (bySeverity.critical > 0) {
        console.log(
          `🔴 关键问题: ${colors.red}${bySeverity.critical}${colors.reset}`
        );
      }
      if (bySeverity.high > 0) {
        console.log(
          `🟠 高优先级: ${colors.yellow}${bySeverity.high}${colors.reset}`
        );
      }
    }
  ...
  printHealthBar(score: number) {
    ...
    const color =
      score >= 85 ? colors.green : score >= 70 ? colors.yellow : colors.red;

    console.log(
      `[${color}${'█'.repeat(filledLength)}${colors.reset}${'░'.repeat(emptyLength)}]`
    );
  }
  ...
  generateImprovementSuggestions(): string[] {
    ...
    const failedAPIs = this.results.filter(
      (r) => r.category === 'API接口' && r.status === 'FAIL'
    );
    ...
    const performanceWarnings = this.results.filter(
      (r) => r.category === '性能优化' && r.status === 'WARNING'
    );
    ...
    const uxWarnings = this.results.filter(
      (r) => r.category === '用户体验' && r.status === 'WARNING'
    );
...
`;
    ...
    console.log(
      `📝 Markdown报告已保存: ${colors.blue}${mdPath}${colors.reset}`
    );
  }
}
