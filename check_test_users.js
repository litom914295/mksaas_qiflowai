...
async function checkAndCreateTestUsers() {
  ...
  // 同样检查 demo@mksaas.com
  const demoUser = authUsers?.users.find((u) => u.email === 'demo@mksaas.com');

  if (!demoUser) {
    console.log('\n创建 demo@mksaas.com 测试用户...');
    ...
    console.log('\n✓ demo@mksaas.com 用户已存在');
  }

  // 检查 admin@mksaas.com 用户
  const adminUser = authUsers?.users.find(
    (u) => u.email === 'admin@mksaas.com'
  );

  if (adminUser) {
    console.log('\n✓ admin@mksaas.com 用户已存在，测试登录...');
