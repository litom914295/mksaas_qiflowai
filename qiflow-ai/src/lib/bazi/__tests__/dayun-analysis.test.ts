...
async function testDayunAnalysis() {
  ...
  try {
    ...
    if (baziResult) {
      ...
      const tenGodAnalysis = tenGodsCalculator.calculateTenGods(baziResult.pillars);
      
      console.log('✅ 十神分析完成');
      console.log(`   日主: ${tenGodAnalysis.dayMaster}`);
      console.log(`   主导十神: ${tenGodAnalysis.dominantGods.join('、')}`);
      console.log(`   性格优势: ${tenGodAnalysis.personality.strengths.slice(0, 3).join('、')}`);
    ...
    if (allLuckPillars.length > 0) {
      ...
      console.log(`   天干地支: ${firstDayun.pillar.heavenlyStem}${firstDayun.pillar.earthlyBranch}`);
      ...
      console.log(`   组合影响: ${firstDayun.tenGodRelation.combinedInfluence}`);
      
      // 各方面影响
      if (firstDayun.tenGodRelation.personalityImpact.length > 0) {
        console.log(`   性格影响: ${firstDayun.tenGodRelation.personalityImpact[0]}`);
      }
      if (firstDayun.tenGodRelation.careerImpact.length > 0) {
        console.log(`   事业影响: ${firstDayun.tenGodRelation.careerImpact[0]}`);
      }
      if (firstDayun.tenGodRelation.wealthImpact.length > 0) {
        console.log(`   财运影响: ${firstDayun.tenGodRelation.wealthImpact[0]}`);
      }
      
      // 重大事件预测
      console.log(`   重大事件预测: ${firstDayun.majorEvents.length}个事件`);
      firstDayun.majorEvents.forEach((event, index) => {
        console.log(`     ${index + 1}. ${event.year}年(${event.age}岁): ${event.description}`);
      });
      
      // 流年互动
      console.log(`   流年互动分析: ${firstDayun.yearlyInteractions.length}个关键年份`);
      firstDayun.yearlyInteractions.slice(0, 3).forEach((interaction, index) => {
        console.log(`     ${index + 1}. ${interaction.year}年: ${interaction.description}`);
      });
    ...
    if (currentDayun) {
      ...
      console.log(`   当前大运: ${currentDayun.pillar.heavenlyStem}${currentDayun.pillar.earthlyBranch}`);
      console.log(`   关键特征: ${currentDayun.keyThemes.slice(0, 3).join('、')}`);
