import { AIRequest, AIResponse, ProviderClient, ProviderConfig, AIMessage } from '../types';
...
function sanitizeMessage(message: AIMessage): AIMessage {
  ...
  // 验证内容长度
  if (typeof content !== 'string' || content.length > SECURITY_CONFIG.MAX_CONTENT_LENGTH) {
...
const mapToOpenAI = (messages: AIMessage[]) => {
  ...
  if (messages.length > SECURITY_CONFIG.MAX_MESSAGES) {
    throw new Error(`消息数量超出限制: ${messages.length}/${SECURITY_CONFIG.MAX_MESSAGES}`);
  }
  
  return messages
    .map(sanitizeMessage)
    .filter(m => m.content.length > 0); // 过滤空消息
};
...
function calculateRetryDelay(attempt: number): number {
  return SECURITY_CONFIG.RETRY_DELAY_BASE * Math.pow(2, attempt) + Math.random() * 1000;
}
