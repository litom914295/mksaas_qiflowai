    ...
    try {
      ...
      if (
        ...
        try {
          ...
          } else if (baziResponse.needsClarification) {
            // 需要澄清信息
            return {
              aiResponse: {
...
`;
      ...
      if (fengShuiResult && fengShuiResult.data) {
    ...
    keywords.forEach(keyword => {
      switch (keyword) {
        ...
        case '飞星':
          ...
          break;
        case '天干':
          response += `🔤 **天干**：甲、乙、丙、丁、戊、己、庚、辛、壬、癸十个符号，代表阳性能量。\n\n`;
          break;
        case '地支':
          response += `🐭 **地支**：子、丑、寅、卯、辰、巳、午、未、申、酉、戌、亥十二个符号，代表阴性能量。\n\n`;
          break;
      }
    });
