...
export class KnowledgeBase {
  private static readonly FENGSHUI_CONTENT: Record<string, EducationalContent> = {
    'basic_principles': {
      ...
      domain: 'fengshui',
      content: {
        definition: '风水是研究人与环境关系的传统学问，旨在通过合理规划居住和工作环境，达到趋吉避凶的目的。',
        explanation: '风水理论认为，自然环境中存在着一种看不见的能量场（气），这种能量场会影响人的健康、运势和心理状态。通过合理的空间布局和方位选择，可以调节和优化这种能量场，从而改善居住者的生活质量。',
        examples: [
          '选择房屋时，优先考虑采光充足、通风良好的位置',
          '客厅布置时，避免横梁压顶，保持空间的开阔感',
          '卧室门不要直对厕所门，保持空间的清洁能量'
        ],
        practicalApplication: '现代风水实践更注重科学性和实用性，结合建筑学、环境学和心理学的原理，为居住者创造舒适健康的生活环境。',
  ...
  private static readonly BAZI_CONTENT: Record<string, EducationalContent> = {
    'four_pillars': {
      ...
      domain: 'bazi',
      content: {
        definition: '四柱八字是中国传统命理学的核心理论，通过一个人出生的年、月、日、时的天干地支组合，分析个人性格特征和人生趋势。',
        explanation: '四柱分别代表：年柱（祖辈、早年），月柱（父母、青年），日柱（自己、中年），时柱（子女、晚年）。每柱由一个天干和一个地支组成，共八个字，故称"八字"。通过分析这八个字的五行属性、相互关系，可以了解一个人的基本性格和发展倾向。',
        examples: [
          '甲子年：甲为阳木，子为阳水，水生木，年份能量和谐',
          '丙午月：丙为阳火，午为阳火，火旺之月，性格偏热情',
          '戊戌日：戊为阳土，戌为阳土，土旺之人，性格稳重'
        ],
        practicalApplication: '现代应用中，八字分析主要用于了解个人性格特点，选择适合的发展方向，以及在重要决策时提供参考。',
        commonMisconceptions: [
          '误解一：八字决定一切，命运无法改变',
          '误解二：八字可以精确预测具体事件',
          '误解三：缺什么五行就要补什么'
  ...
  static getContent(domain: string, topic: string): EducationalContent | null {
    const contentMap = domain === 'fengshui' ? this.FENGSHUI_CONTENT : this.BAZI_CONTENT;
  ...
  static searchContent(query: string, domain?: string): EducationalContent[] {
    const allContent = [
      ...Object.values(this.FENGSHUI_CONTENT),
      ...Object.values(this.BAZI_CONTENT)
    ];

    return allContent.filter(content => {
      if (domain && content.domain !== domain) return false;
      
      const searchText = `${content.title} ${content.content.definition} ${content.content.explanation}`.toLowerCase();
  ...
  static getLearningPath(id: string): LearningPath | null {
    return this.LEARNING_PATHS.find(path => path.id === id) || null;
  }
  ...
  static recommendLearningPath(
    ...
    return this.LEARNING_PATHS.filter(path => 
      path.domain === domain && 
...
export class EducationService {
  ...
  generateEducationalResponse(
    ...
    questionType: 'explanation' | 'example' | 'application' | 'quiz' = 'explanation'
  ): string {
    ...
    return this.formatEducationalContent(content, questionType, expertise, style);
  }
  ...
  private formatEducationalContent(
    ...
    // 添加常见误解
    if (style === 'detailed' && content.content.commonMisconceptions.length > 0) {
      response += `## ⚠️ 常见误解\n`;
  ...
  generateLearningRecommendation(
    ...
    const recommendedPaths = KnowledgeBase.recommendLearningPath(domain, expertise);
