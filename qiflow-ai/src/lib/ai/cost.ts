...
interface CostStorage {
  ...
  setUsage(userId: string, usage: { daily: number; monthly: number }): Promise<void>;
...
class MemoryCostStorage implements CostStorage {
  ...
  async getUsage(userId: string): Promise<{ daily: number; monthly: number } | null> {
    return this.storage.get(userId) || null;
  }

  async setUsage(userId: string, usage: { daily: number; monthly: number }): Promise<void> {
...
class DatabaseCostStorage implements CostStorage {
  ...
  async getUsage(userId: string): Promise<{ daily: number; monthly: number } | null> {
    ...
    try {
      ...
      const daily = dailyData?.reduce((sum: number, record: any) => sum + record.cost_usd, 0) || 0;
      const monthly = monthlyData?.reduce((sum: number, record: any) => sum + record.cost_usd, 0) || 0;
...
// 根据环境选择存储实现
const costStorage: CostStorage = process.env.NODE_ENV === 'production' 
  ? new DatabaseCostStorage() 
  : new MemoryCostStorage();

export const estimateCostUsd = (model: string, promptTokens: number, completionTokens: number): number => {
...
export const checkBudget = async (userId: string | undefined, budget?: CostBudget): Promise<boolean> => {
...
export const recordUsage = async (userId: string | undefined, cost: number): Promise<void> => {
  if (!userId) return;
  await costStorage.incrementUsage(userId, cost);
};

// 新增：获取用户使用情况
export const getUserUsage = async (userId: string): Promise<{ daily: number; monthly: number }> => {
  const usage = await costStorage.getUsage(userId);
  return usage || { daily: 0, monthly: 0 };
};

// 新增：获取用户预算状态
export const getUserBudgetStatus = async (userId: string, budget: CostBudget): Promise<{
  ...
  const monthlyPercentage = monthlyLimit > 0 ? (usage.monthly / monthlyLimit) * 100 : 0;
