...
import type { ConfidenceScore, AIResponse, TermExplanation } from './types';
...
export class ConfidenceEvaluator {
   ...
   * 计算整体置信度分数
   */
  evaluateConfidence(input: AnalysisInput, aiResponse: AIResponse): ConfidenceScore {
   ...
   * 评估理论匹配度
   */
  private evaluateTheoryMatch(input: AnalysisInput, aiResponse: AIResponse): number {
    let score = 0.6; // 基础分
  ...
  evaluateFengShuiAnalysis(
    ...
    const culturalTermsCount = this.countCulturalTerms(aiResponse.choices[0]?.message?.content || '');
   ...
   * 评估风水分析复杂度
   */
  private assessFengShuiComplexity(flyingStarData: any, houseData: any): 'simple' | 'medium' | 'complex' {
  ...
  private assessBaziComplexity(baziData: any): 'simple' | 'medium' | 'complex' {
    ...
    if (baziData?.elements && Object.keys(baziData.elements).length === 5) complexityScore += 1;
 ...
 * 生成置信度警告
 */
export function generateConfidenceWarning(score: ConfidenceScore): string | null {
