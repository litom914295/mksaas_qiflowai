import { Mountain, FlyingStar, Yun, PalaceIndex, Plate, LingzhengAnalysis } from './types';
import { getPalaceByMountain, PALACE_TO_BAGUA } from './luoshu';
...
export const LINGZHENG_PERIOD_TABLE: LingzhengPeriodInfo[] = [
  { period: 1, positiveGod: 1, zeroGod: 7, description: '一运：一白为正神，七赤为零神' },
  { period: 2, positiveGod: 2, zeroGod: 8, description: '二运：二黑为正神，八白为零神' },
  { period: 3, positiveGod: 3, zeroGod: 9, description: '三运：三碧为正神，九紫为零神' },
  { period: 4, positiveGod: 4, zeroGod: 1, description: '四运：四绿为正神，一白为零神' },
  { period: 5, positiveGod: 5, zeroGod: 2, description: '五运：五黄为正神，二黑为零神' },
  { period: 6, positiveGod: 6, zeroGod: 3, description: '六运：六白为正神，三碧为零神' },
  { period: 7, positiveGod: 7, zeroGod: 4, description: '七运：七赤为正神，四绿为零神' },
  { period: 8, positiveGod: 8, zeroGod: 5, description: '八运：八白为正神，五黄为零神' },
  { period: 9, positiveGod: 9, zeroGod: 6, description: '九运：九紫为正神，六白为零神' }
];
...
// 分析飞星盘中的零正神位置
export function analyzeLingzhengPositions(plate: Plate, period: Yun): {
  zeroGodPositions: PalaceIndex[];
  positiveGodPositions: PalaceIndex[];
  zeroGodDetails: { palace: PalaceIndex; bagua: string; starType: 'period' | 'mountain' | 'facing' }[];
  positiveGodDetails: { palace: PalaceIndex; bagua: string; starType: 'period' | 'mountain' | 'facing' }[];
} {
  const lingzhengInfo = getLingzhengInfo(period);
  const zeroGodPositions: PalaceIndex[] = [];
  const positiveGodPositions: PalaceIndex[] = [];
  const zeroGodDetails: { palace: PalaceIndex; bagua: string; starType: 'period' | 'mountain' | 'facing' }[] = [];
  const positiveGodDetails: { palace: PalaceIndex; bagua: string; starType: 'period' | 'mountain' | 'facing' }[] = [];
  ...
  for (const cell of plate) {
    ...
    if (cell.periodStar === lingzhengInfo.positiveGod) {
      ...
      positiveGodDetails.push({ palace: cell.palace, bagua, starType: 'period' });
    }
    if (cell.mountainStar === lingzhengInfo.positiveGod) {
      if (!positiveGodPositions.includes(cell.palace)) {
        positiveGodPositions.push(cell.palace);
      }
      positiveGodDetails.push({ palace: cell.palace, bagua, starType: 'mountain' });
    }
    if (cell.facingStar === lingzhengInfo.positiveGod) {
      if (!positiveGodPositions.includes(cell.palace)) {
        positiveGodPositions.push(cell.palace);
      }
      positiveGodDetails.push({ palace: cell.palace, bagua, starType: 'facing' });
    }
  }
...
export function checkZeroPositiveReversed(
  ...
  // 检查零神是否见山（凶）
  const zeroGodSeesMountain = lingzhengPositions.zeroGodPositions.some(palace =>
    mountainPositions.includes(palace)
  );
  
  // 检查正神是否见水（凶）
  const positiveGodSeesWater = lingzhengPositions.positiveGodPositions.some(palace =>
    waterPositions.includes(palace)
  );
  ...
  if (zeroGodSeesMountain) {
    ...
    issues.push(`零神见山：${affectedPalaces.join('、')}宫零神见山，主破财损丁`);
  }
  ...
  if (positiveGodSeesWater) {
    const affectedPalaces = lingzhengPositions.positiveGodPositions
      ...
      .map(palace => PALACE_TO_BAGUA[palace]);
    issues.push(`正神见水：${affectedPalaces.join('、')}宫正神见水，主散财败家`);
  }
...
export function analyzeLingzhengTimeChange(
  ...
  // 生成分析结果
  const currentRecommendations = generateLingzhengRecommendations(plate, currentPeriod, '子', '午');
  const futureRecommendations = generateLingzhengRecommendations(plate, futurePeriod, '子', '午');
...
export function analyzeLingzheng(
  ...
  const positions = analyzeLingzhengPositions(plate, period);
  const recommendations = generateLingzhengRecommendations(plate, period, zuo, xiang);
  const reversedCheck = checkZeroPositiveReversed(plate, period, zuo, xiang, environmentInfo);
