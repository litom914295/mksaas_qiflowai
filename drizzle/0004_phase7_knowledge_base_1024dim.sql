-- Phase 7: RAG 知识库表（硅基流动 BAAI/bge-m3 适配版）
...
CREATE TABLE IF NOT EXISTS "knowledge_documents" (
  ...
  -- 文档信息
  "title" text NOT NULL,
  "category" text NOT NULL,  -- 'bazi' | 'fengshui' | 'faq' | 'case' | 'general'
  "source" text NOT NULL,    -- 来源书籍/网站名称
  "content" text NOT NULL,
  
  -- 元数据
  "metadata" jsonb DEFAULT '{}',  -- { author, year, page, tags, language, chunk_index, total_chunks }
  
  -- 向量 (1024 维 - 硅基流动 BAAI/bge-m3)
...
-- 添加注释
COMMENT ON TABLE "knowledge_documents" IS 'RAG 知识库文档表，存储向量化的八字风水知识（BAAI/bge-m3, 1024维）';
COMMENT ON COLUMN "knowledge_documents"."category" IS '文档分类: bazi(八字) / fengshui(风水) / faq(常见问题) / case(案例) / general(通用)';
COMMENT ON COLUMN "knowledge_documents"."embedding" IS '硅基流动 BAAI/bge-m3 生成的 1024 维向量';
...
COMMENT ON COLUMN "rag_retrieval_logs"."query_embedding" IS '查询文本的向量表示（1024维）';
...
COMMENT ON FUNCTION search_knowledge_documents IS 'RAG 向量相似度搜索函数，支持分类过滤和相似度阈值（1024维）';
