...
async function gotoSignUp(page) {
  ...
  const visible = await page.locator('#email').isVisible().catch(() => false)
  if (!visible) {
...
test.describe('Growth Activation E2E', () => {
  test('登记推荐 → 业务完成 → 激活发放（幂等）', async ({ browser }) => {
    ...
    const act1 = await pageB.request.post('/api/referral/activate', { data: {} })
    ...
    const act2 = await pageB.request.post('/api/referral/activate', { data: {} })
