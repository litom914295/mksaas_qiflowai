...
test.describe('玄空风水分析 E2E 测试', () => {
  ...
  test('完整分析流程 - 从输入到结果展示', async ({ page }) => {
    ...
    await test.step('配置分析选项', async () => {
      ...
      // 添加环境信息（可选）
      const addEnvButton = await page.locator('button:has-text("添加环境信息")');
    ...
    await test.step('验证基础分析结果', async () => {
      // 检查飞星盘显示
      const flyingStarPlate = await page.locator('[data-testid="flying-star-plate"]');
      await expect(flyingStarPlate).toBeVisible();
      
      // 检查九宫格
      const palaces = await page.locator('[data-testid="palace-grid"] .palace-cell');
    ...
    await test.step('验证七星打劫分析', async () => {
      const qixingSection = await page.locator('[data-testid="qixing-dajie-analysis"]');
      ...
      if (await qixingSection.isVisible()) {
        ...
        // 检查激活条件
        const requirements = await qixingSection.locator('[data-testid="activation-requirements"]');
    ...
    await test.step('验证零正理论分析', async () => {
      const lingzhengSection = await page.locator('[data-testid="lingzheng-analysis"]');
      
      if (await lingzhengSection.isVisible()) {
        // 检查零神位和正神位
        const zeroGodPosition = await lingzhengSection.locator('[data-testid="zero-god-position"]');
        await expect(zeroGodPosition).toBeVisible();
        
        const positiveGodPosition = await lingzhengSection.locator('[data-testid="positive-god-position"]');
        await expect(positiveGodPosition).toBeVisible();
        
        // 检查零正颠倒警告（如果存在）
        const reverseWarning = await lingzhengSection.locator('.alert-destructive');
    ...
    await test.step('验证城门诀分析', async () => {
      const chengmenSection = await page.locator('[data-testid="chengmenjue-analysis"]');
      
      if (await chengmenSection.isVisible()) {
        // 检查城门位置
        const positions = await chengmenSection.locator('[data-testid="chengmen-positions"]');
        ...
        // 检查催旺方法
        const methods = await chengmenSection.locator('[data-testid="activation-methods"]');
    ...
    await test.step('验证化解方案展示', async () => {
      const remedySection = await page.locator('[data-testid="remedy-solutions"]');
      
      if (await remedySection.isVisible()) {
        // 检查是否有方案卡片
        const solutionCards = await remedySection.locator('[data-testid="solution-card"]');
        expect(await solutionCards.count()).toBeGreaterThan(0);
        
        // 点击查看详情
        const firstCard = solutionCards.first();
        const detailButton = await firstCard.locator('button:has-text("查看详情")');
