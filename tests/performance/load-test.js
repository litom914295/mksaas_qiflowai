...
export function testAPIEndpoint() {
  ...
  const success = check(response, {
    ...
    API返回JSON: (r) => r.headers['Content-Type']?.includes('application/json'),
  });
...
function textSummary(data, options) {
  ...
  if (metrics) {
    summary.push(
      `${indent}总请求数: ${metrics.http_reqs?.values?.count || 0}\n`
    );
    summary.push(
      `${indent}失败请求: ${metrics.http_req_failed?.values?.passes || 0}\n`
    );
    summary.push(
      `${indent}平均响应时间: ${Math.round(metrics.http_req_duration?.values?.avg || 0)}ms\n`
    );
    summary.push(
      `${indent}P95响应时间: ${Math.round(metrics.http_req_duration?.values?.p95 || 0)}ms\n`
    );
  }
  ...
  summary.push(`\n${indent}=== 自定义指标 ===\n`);
  summary.push(
    `${indent}错误率: ${Math.round((metrics.errors?.values?.rate || 0) * 100)}%\n`
  );
  summary.push(
    `${indent}API平均响应: ${Math.round(metrics.api_response_time?.values?.avg || 0)}ms\n`
  );
  summary.push(
    `${indent}页面平均加载: ${Math.round(metrics.page_load_time?.values?.avg || 0)}ms\n`
  );
