...
```env
# 数据库
DATABASE_URL=postgresql://user:password@localhost:5432/mksaas_db
...
# 或者手动执行 SQL
psql -U postgres -d mksaas_db -f src/db/migrations/001_growth_system.sql
```
...
# 或手动在数据库中插入
psql -U postgres -d mksaas_db
```

```sql
-- 插入管理员账号
INSERT INTO users (id, email, name, password, role, status) VALUES
(gen_random_uuid(), 'admin@mksaas.com', '超级管理员', '$2a$10$hashedpassword', 'admin', 'active'),
(gen_random_uuid(), 'manager@mksaas.com', '运营管理员', '$2a$10$hashedpassword', 'manager', 'active');
```
...
```bash
# 使用命令行重置密码
pnpm admin:reset-password admin@mksaas.com
```
...
如需帮助，请：
- 📧 发送邮件至: support@mksaas.com
- 💬 在GitHub提交Issue
- 📖 查看完整文档: https://docs.mksaas.com
