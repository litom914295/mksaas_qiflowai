...
test.describe('国际化路由导航', () => {
  test.describe('无 locale 路径重定向', () => {
    ...
    test('访问 /analysis/bazi 应该重定向到 /zh-CN/analysis/bazi', async ({ page }) => {
      await page.goto('/analysis/bazi');
      await expect(page).toHaveURL(/\/zh-CN\/analysis\/bazi/);
    });

    test('访问 /analysis/xuankong 应该重定向到 /zh-CN/analysis/xuankong', async ({ page }) => {
  ...
  test.describe('语言切换', () => {
    test('切换到英文应该更新 URL locale', async ({ page }) => {
      ...
      // 查找语言切换器（假设有这个功能）
      const langSwitcher = page.locator('[data-testid="language-switcher"], [aria-label*="language"], button:has-text("EN")');
      
      if (await langSwitcher.count() > 0) {
        await langSwitcher.click();
        
        // 如果有语言选项，点击英文
        const enOption = page.locator('[data-locale="en"], a:has-text("English")');
