...
- k8s/ingress-canary.yaml：Canary Ingress（指向 qiflowai-service-dr，初始权重 0）
...
```bash
kubectl -n qiflowai patch ingress qiflowai-ingress-canary \
...
```bash
curl -X POST http://traffic-switcher.qiflowai.svc.cluster.local/switch \
